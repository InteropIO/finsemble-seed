...
// special finsemble component to display table data in JSON format; incoked by LoggerMainConsoleTable component
const JSONFormatter = __webpack_require__(235).default;

const HIGHLIGH_STYLE_COLOR1 = ["red", "blue", "green", "purple"];
const HIGHLIGH_STYLE_COLOR2 = ["yellow", "yellow", "yellow", "yellow"];
...
function addDataHighlights(highlightString, data) {
	// string input
	var workingData = data;
	for (var i = 0; i < highlightString.length; i++) {
		let searchString = highlightString[i];
		if (searchString && searchString.length > 0) {
			if (workingData && typeof workingData === "string") {
				let result = workingData.indexOf(searchString);
				if (result !== -1) {
					let replacement = "<span style='color:" + HIGHLIGH_STYLE_COLOR1[i] + ";'>" + searchString + "</span>";
					let newString = workingData.split(searchString).join(replacement);
					workingData = newString;
				}
			}
		}
	}
...
function ready() {
	fin.desktop.Window.getCurrent().getOptions(function (opts) {
		var logData = opts.customData.spawnData.logData;
		var startIndex = opts.customData.spawnData.startIndex;
		var stopIndex = opts.customData.spawnData.stopIndex;
		var highlightString = opts.customData.spawnData.highlightString;
		var viewDepth = opts.customData.spawnData.viewDepth;
		console.log("JSONFormatter", logData, startIndex, stopIndex);

		let div = document.createElement("div");
		div.innerHTML = "<h3>" + "Central Log Data for Rows " + (startIndex + 1) + " to " + (stopIndex + 1) + "</h3><hr>";
		document.body.appendChild(div);

		for (var i = startIndex; i <= stopIndex; i++) {
			let type = logData[i].logType;
			let clientName = logData[i].logClientName;
			let timeStamp = logData[i].logTimestamp;
			let div1 = document.createElement("div");
			div1.style.color = typeColor(type);
			div1.innerHTML = "<hr/><span>" + i + " " + clientName + " " + type + " " + timeStamp + "</span>";
			document.body.appendChild(div1);

			let highlightedData = addDataHighlights(highlightString, logData[i].allLogArgs);
			let div2 = document.createElement("div");
			div2.innerHTML = "<span> 0: " + highlightedData + "</span>";
			document.body.appendChild(div2);

			if (!(logData[i].logArgs.length === 1 && typeof logData[i].parsedLogArgs[0] === "string") && viewDepth > 0) {
				let div = document.createElement("div");
				div.innerHTML = "<br><span><b>" + "Object Format:" + "</b></span>";
				document.body.appendChild(div);

				let formatedDataLine = new JSONFormatter(logData[i].parsedLogArgs, viewDepth, {
				...
				document.body.appendChild(formatedDataLine.render());
			}
		}
	});
}
...
/******/ });
//# sourceMappingURL=JSONAllData.js.map