...
* All rights reserved.
*
*Configs are set in finsemble.betaFeatures.assimilation.enabled
*/
//
var RouterClient = __webpack_require__(4);
var baseService = __webpack_require__(14);
var WindowModel = __webpack_require__(51);
...
Logger.system.log("Starting Assimilation Service");
...
function AssimilationService() {
	var self = this;

	var activeWindows = {};
	var allWindows = {}; /// a list of all windows on the os.
	var ws = null;
	var port = 8392;
	...
	window.external = activeWindows;
	var visibleWindows = {};
	this.routerClient = RouterClient;
	this.windowGroups = {};
	this.scalingFactor = 1;
	window.list = this.activeWindows;
	var currentMoving = null;
	var onlySpawnedWindows = true;
	var blacklist = [""]; //name in this list
	var whitelist = []; //name in this list
	var ready = false; /// we use this to tag windows before we load windows from the workspace
	var readyFunction = null;

	/**
 *
 This registers a window with the docker service
 * @param  {Object} windowObject Our window object(windowModel)
 */
	function registerWindow(windowMsg, windowObject) {
		self.activeWindows[windowMsg.uuid] = windowObject; //Save our window
		if (componentsSpawnedByApp[windowMsg.name]) {
			windowMsg.externalSpawnedByApp = true;
		}
		self.routerClient.query("register-docking-window", { type: "external", params: windowMsg }, function () {});
		var output = JSON.stringify({
			topic: "hookWindow",
			uuid: windowObject.uuid

		});
		Logger.system.log("hook window", windowMsg.name);
		ws.send(output);
	}

	/**
 * Remove a window from a docker group
 * @param  {String} windowName the name of the window that is going to leave the group
 @param  {Function} cb callkback
 */
	function leaveGroup(windowName, cb) {
		self.routerClient.query("DockingService.leaveGroup", { type: "external", name: windowName }, function () {
			cb();
		});
	}

	/**
 * Remove a window from the docker
 * @param  {String} windowName the name of the window that is going to ne removed from the docker service
 */
	function unRegisterWindow(windowName) {
		self.routerClient.query("deregister-docking-window", { name: key }, function () {});
	}

	/**
 * This will trun on our c++ application and connect.
 * @param  {Function} cb
 */
	this.activateExternalApp = function () {
		launchAssimilation(function (err) {
			if (err) {
				Logger.system.log("Err", err);
				return;
			}
			connectSockets(); //Connect to socket
		});
	};
  ...
  * @Todo write this
  */
	this.deactivateExternalApp = function () {
		//first part will launch the c++ app
		ws.close(); //Not sure if this is correct
	};

	// This is what allows external applications to work with Finsemble. If the hooks are off, we don't receive the move events
	this.enableAppHooks = function () {
		Logger.system.log("enable hook");
  ...
  * @todo
  */
	this.addBlackListedWindow = function () {
		ws.send(JSON.stringify({
			topic: "addBlackListedWindow",
			data: {
				whitelist: "someTopic",
				blacklist: ""
			}
	...
	/**
 * Thi turns off our hooks into each window aka we won't catch move events anymore.
 */
	this.disableAppHooks = function () {
		ws.send(JSON.stringify({
			topic: "disableHooks",
			data: {}
		}));
  ...
  */
	this.load = function (cb) {
		...
		ConfigClient.get({ field: "finsemble.betaFeatures.assimilation" }, function (err, data) {
			//Check our config
			if (err) {
				return Logger.system.error(err);
			}
			...
			blacklist = data.blacklist.map(function (x) {
				return x.toUpperCase();
			});
			whitelist = data.whitelist.map(function (x) {
				return x.toUpperCase();
			});
			onlySpawnedWindows = data.onlySpawned;

			fin.desktop.System.getMonitorInfo(monitorInfo => {
				self.scalingFactor = monitorInfo.deviceScaleFactor; //We need this. Info from the c++application isnt scaled
				setupWindowListeners();
				if (data.enabled) {
					readyFunction = cb;
					return self.activateExternalApp(); //Turn on the application
				}
				ready = true;
				cb();
			});
	...
	/**
  * Remove all windows from the docking services.
  */
	this.unRegisterAll = function () {
		var windowsize = Object.keys(self.activeWindows).length;
		if (windowsize > 0) {
			for (key in self.activeWindows) {
	...
	/**
  * This is where we handle the connection to the c++ app
  */
	function connectSockets() {
		// Connect to the c++ app
		if (ws && (ws.readyState === 0 || ws.readyState === 1)) {
			return;
		} //If we're connected don't try to connect again
		ws = new WebSocket("ws://127.0.0.1:" + port);
		Logger.system.log("connecting to :", "ws://127.0.0.1:" + port);
		ws.onopen = function () {
			Logger.system.log("connected to socket");
			self.enableAppHooks();
			Logger.system.log("sending get list");
			ws.send(JSON.stringify({ //Get a list of windows
		...
		ws.onmessage = function (msg) {
			...
			try {
				...
				Logger.system.log("msg err", err, msg);
				return;
			}
			if (!parsedMessage.topic) {
				return;
			}
			if (!socketMessages[parsedMessage.topic]) {
				return;
			}
			socketMessages[parsedMessage.topic](parsedMessage);

			//msg.data === data
		};

		ws.onclose = function () {
			Logger.system.log("web socket closed");
			self.unRegisterAll();
			connectSockets();
		};

		socketMessages = { //We should make all of these funcions outside of this object
			"startMove": function (msg) {
				//Signal move start
				Logger.system.log("start move", msg);
				if (!msg.uuid) {
					Logger.system.log("window is missing uuid", msg);
					return;
				}
				currentMoving = msg.uuid;
				setupWindowWrapper(msg);
			},
			"canHookWindow": function (msg) {},
			"updateWindowList": function (msg) {
				//Not used yet
				if (msg.windows) {
					Logger.system.log("list is updated", msg);
				}
			},
			"applicationList": function (msg) {
				//gets a full updated list of windows

				parseApplicationList(msg.list);
			},
			"keyDown": function (msg) {
				RouterClient.publish("humanInterface.keydown", { key: msg.key }); //receive key down
				if (msg.key = "Shift") {
					self.shift = true;
				}
			},
			"keyUp": function (msg) {
				RouterClient.publish("humanInterface.keyup", { key: msg.key }); //receive key up
				if (msg.key = "Shift") {
					self.shift = false;
				}
			},
			"moveEnd": function (msg) {
				//Movement by user is complete...mouse up.
				if (!currentMoving) {
					return;
				}
				if (!self.activeWindows[currentMoving]) {
					return;
				}
				self.activeWindows[currentMoving].calculateWindowLocation(msg);

				if (self.activeWindows[currentMoving].spawned) {
					//update wrokspace info here
					self.activeWindows[currentMoving].spawned.params.defaultLeft = self.activeWindows[currentMoving].location.left;
					self.activeWindows[currentMoving].spawned.params.defaultTop = self.activeWindows[currentMoving].location.top;
					self.activeWindows[currentMoving].spawned.params.defaultWidth = self.activeWindows[currentMoving].location.width;
					self.activeWindows[currentMoving].spawned.params.defaultHeight = self.activeWindows[currentMoving].location.height;
					let windowHash = Utils.camelCase("activeWorkspace", self.activeWindows[currentMoving].spawned.params.name);
					StorageClient.save({
						topic: "finsemble",
						key: windowHash,
						value: self.activeWindows[currentMoving].spawned.params
					});
				}
				msg.location = self.activeWindows[currentMoving].location;
				self.routerClient.transmit(currentMoving + ".endMovement", msg);
				self.activeWindows[currentMoving].edge = null;
				currentMoving = null;
			},
			"moveUpdate": function (msg) {
				//Move event received.
				//Logger.system.log("move update", currentMoving);
				var currentWindow = self.activeWindows[currentMoving];
				if (!currentWindow) {
					Logger.system.log("no window found");
				...
				self.activeWindows[currentMoving].calculateWindowLocation(msg);
				if (self.shift) {
					//If shift is down check if this window needs to leave a group
					self.activeWindows[currentMoving].shift = true;
					for (var key in self.windowGroups) {
						var group = self.windowGroups[key];
						for (var i = 0; i < group.length; i++) {
							if (group[i] === currentMoving) {
								return leaveGroup(currentMoving, function (err, message) {
									if (err) {
										return Logger.system.error(err);
									}
									if (!message) {
									...
									self.windowGroups = message.data;
									self.routerClient.transmit(currentMoving + ".move", {
										mouseLocation: self.activeWindows[currentMoving].mouseLocation,
										location: self.activeWindows[currentMoving].location,
										uuid: self.activeWindows[currentMoving].uuid
									});
								});
							}
						}
				...
				self.routerClient.transmit(currentMoving + ".move", //transmit the move event to the docker
				{
					mouseLocation: self.activeWindows[currentMoving].mouseLocation,
					location: self.activeWindows[currentMoving].location,
					uuid: self.activeWindows[currentMoving].uuid
			...
			"resize": function (msg) {},
			"windowOpen": function (msg) {},
			"windowClose": function (msg) {},
			"connected": function (msg) {
				Logger.system.log("message connected", msg);
			}
		};
	}

	//Sets up our window wrapper and tracks our windows. This will also attach any spawned components to the window info. They are connected by exe.
	function setupWindowWrapper(msg) {
		var movingWindow;
		var needsToRegister = false;
		if (!self.activeWindows[msg.uuid]) {
			needsToRegister = true;
			movingWindow = new WindowModel();
			movingWindow.registered = false;
		} else {
			//return self.activeWindows[msg.uuid];
			movingWindow = self.activeWindows[msg.uuid];
		}

		movingWindow.executablePath = msg.executablePath;
		movingWindow.state = movingWindow.state ? movingWindow.state : msg.state;
		movingWindow.preSpawn = ready ? false : true;
		movingWindow.name = msg.name;
		var spawnedItem = findSpawnedWindow(movingWindow);

		movingWindow.setScalingFactor(self.scalingFactor);
		movingWindow.setStartBounds(msg); // = startBounds;
		movingWindow.setUUID(msg.uuid);
		movingWindow.setResizeType(msg.resizeType);
		movingWindow.setMouseLocation(msg);
		movingWindow.setMouseOffset(movingWindow.startBounds);
		movingWindow.isMoving = true;
		movingWindow.setLocation(movingWindow.startBounds);

		if (spawnedItem && !movingWindow.spawned) {

			movingWindow.spawned = spawnedItem;
			movingWindow.spawned.params.defaultLeft = movingWindow.location.left;
			movingWindow.spawned.params.defaultTop = movingWindow.location.top;
			movingWindow.spawned.params.defaultWidth = movingWindow.location.width;
			movingWindow.spawned.params.defaultHeight = movingWindow.location.height;
			let windowHash = Utils.camelCase("activeWorkspace", movingWindow.spawned.params.name);
			StorageClient.save({
				topic: "finsemble",
				key: windowHash,
				value: movingWindow.spawned.params
			});
		}
	...
	//parse the application list passed to use
	function parseApplicationList(list) {
		var parsedList = JSON.parse(list);
		//See if any windows have been removed.
		checkForRemoved(parsedList);

		for (var i = 0; i < parsedList.length; i++) {
			var item = parsedList[i];

			if (activeWindows[item.uuid] || allWindows[item.uuid]) {
				continue;
			} else {

				addWindow(item);
				allWindows[item.uuid] = item;
			}
		}
		if (!ready) {
			ready = true;
	...
	//Check our list to see if any windows have been removed. Remove them if they have been
	function checkForRemoved(list) {
		var found;
		for (key in self.activeWindows) {
			found = false;
			for (var j = 0; j < list.length; j++) {
				if (list[j].uuid === self.activeWindows[key].uuid && !self.activeWindows[key].hidden) {
					found = true;
					break;
				}
			}
			...
			if (!found) {
				unRegisterWindow(self.activeWindows[key].uuid);
				Logger.system.log("remove window", key);
				delete self.activeWindows[key];
			}
		}
	}

	//Check to see if we should be hooking into a window
	function canHookWindow(windowWrap) {

		if (onlySpawnedWindows && !windowWrap.preSpawned && windowWrap.spawned && !checkBlacklist(windowWrap)) {
			return true;
		} // if our config tells us too hook spawned components
		if (onlySpawnedWindows) {
			return false;
		}

		if (checkBlacklist(windowWrap)) {
			return false;
		}
		if (checkWhitelist(windowWrap)) {
			return true;
		}
	...
	function checkBlacklist(windowObject) {
		if (blacklist.length > 0 && blacklist.includes(windowObject.name.toUpperCase())) {
			return true;
		}
		for (var i = 0; i < blacklist.length; i++) {
			if (windowObject.name.toUpperCase().includes(blacklist[i])) {
				return true;
			}
		}
		return false;
	...
	function checkWhitelist(windowObject) {
		if (whitelist.length > 0 && whitelist.includes(windowObject.name.toUpperCase())) {
			return true;
		}
		for (var i = 0; i < whitelist.length; i++) {
			if (windowObject.name.toUpperCase().includes(whitelist[i])) {
				return true;
			}
		}
		return false;
	...
	var excludes = ["Program Manager", "Settings"]; // these seem to get into the list...May need to add more to this list.
	function addWindow(item) {
		if (excludes.includes(item.name)) {
			return;
		}
		//var monitor = Utils.getMonitorFromOpenFinXY(item.left, item.top, function (monitor) {// This doesnt work right now

		if (activeWindows[item.uuid]) {
			return;
		}
		item.x = 1;
		item.y = 1;
		var windowObject = setupWindowWrapper(item);
		var canHook = canHookWindow(windowObject);
		if (!canHook) {
			return;
		}

		if (!windowObject.registered) {
			item.location = windowObject.startBounds;
			registerWindow(item, windowObject); //register the window with the docker if needed
		}
	}
	//Compare a spawned windows exe with one of our live windows. Returns a spawned window if found
	function findSpawnedWindow(params) {
		if (onlySpawnedWindows && params.preSpawned) {
			return;
		}
		if (!params.executablePath) {
			return;
		}
		for (var i = 0; i < self.spawnedWindows.length; i++) {
			if (self.spawnedWindows[i].connected) {
				continue;
			}

			if (params.executablePath.indexOf(self.spawnedWindows[i].target) > -1 && !self.spawnedWindows[i].connected) {
				if (onlySpawnedWindows && !params.preSpawned) {
					self.spawnedWindows[i].connected = true;
				}

				return self.spawnedWindows[i];
			}
		}
	}
	//Compare a spawned windows exe with one of our live windows. Returns an external window if found
	function findActiveWindow(params) {
		//from spawed target
		if (!params.executablePath) {
			return;
		}
		...
		for (var w in self.activeWindows) {
			aWindow = self.activeWindows[w];
			if (aWindow.executablePath.indexOf(params.target) > -1 && !self.spawnedWindows[i].connected) {
				//self.spawnedWindows[i].connected = true;
				if (onlySpawnedWindows && !aWindow.preSpawned) {
					self.spawnedWindows[i].connected = true;
				}
				return aWindow;
			}
	...
	/**
  * Move a window to a desired location
  */
	this.moveWindow = function (params) {
		if (!params.key || !params.location) {
			return;
		}
		let movingWin = self.activeWindows[params.key];
		if (!movingWin) {
			return;
		}
		movingWin.location = Object.assign({}, params.location); //Copy our data
		params.location = Object.assign({}, movingWin.scaleOut(params.location));
	...
	/**
  * Setup our listeners with the docker service
  */
	function setupWindowListeners() {
		RouterClient.addListener("Assimilation.moveWindow", function (err, message) {
			if (err) {
				Logger.system.log("error in move");
				return Logger.system.error(err);
			}
			self.moveWindow(message.data);
		});

		/**
   * This allows use to see what groups windows are in.
   */
		RouterClient.subscribe("DockingService.groupUpdate", (err, response) => {
			self.windowGroups = response.data;
		});

		/**
   * @todo
   */
		RouterClient.addListener("Assimilation.setOpacity", function (err, message) {
			//Logger.system.log("setOpacity");
			//@todo
			if (err) {
				return Logger.system.error(err);
			}
			if (!message.data.key || !message.data.opacity) {
				return;
			}
		});

		/**
   * Show a window. This is used for group resize
   */
		RouterClient.addListener("Assimilation.showWindow", function (err, message) {
			//Logger.system.log()
			if (message.data.location) {}
			//self.moveWindow(message.data);

			//self.activeWindows[message.data.key].location = message.data.location;
			{
				Logger.system.log("send show", message);
			}
			var output = JSON.stringify({
				topic: "showWindow",
		...
		/**
   * Hide a window. This is used for group resize
   */
		RouterClient.addListener("Assimilation.hideWindow", function (err, message) {
			//self.activeWindows[message.data.key].location = message.data.location;
			if (!message.data.key || message.data.key === "") {
				return;
			}
		...
		/**
   * This comes from the launcher. This will spawna  bundled asset
   * @todo make this work with path not just alias
   */
		RouterClient.addResponder("Assimilation.spawnNative", function (err, message) {
			if (err) {
				return Logger.system.error(err);
			}
			self.spawnAsset(message.data, function () {
  ...
  * Spawn our c++ app
  */
	function launchAssimilation(cb) {
		//return cb();
		fin.desktop.System.launchExternalProcess({
			...
			listener: function (event) {
				...
				if (event.topic === "exited") {
					...
					Logger.system.log("Exited Assimilation Process", event);
				} else {
					Logger.system.log("Running Assimilation Process", event);
				}
			}
		}, function (payload) {
			//success
			Logger.system.log("assimilation success", payload.uuid);
			return cb();
		}, function (err) {
			//err
			return cb(err);
	...
	function findAsset(assets, alias) {
  ...
  */
	...
	this.spawnAsset = function (params, cb) {
		ConfigClient.get(null, function (err, manifest) {
			//We can probably move this to the on load
			var foundAsset = findAsset(manifest.appAssets, params.alias || params.path);
			if (params.alias) {
				return spawnByAlias(params, foundAsset, cb);
			}
	...
	function removeSpawnedWindow(uuid) {

		for (var i = 0; i < self.spawnedWindows.length; i++) {
			if (self.spawnedWindows[i].finUUID === uuid) {
				self.spawnedWindows.pop(i);
				return;
			}
		}
	}

	function spawnByPath(params, foundAsset, cb) {
		fin.desktop.System.launchExternalProcess({
			path: params.path,
			...
			listener: function (event) {
				...
				if (event.topic === "exited") {
					// your desired logic here
					WorkspaceClient.removeWindow({ name: params.name });
					removeSpawnedWindow(event.uuid);
					self.routerClient.query("Launcher.removeComponent", { name: params.name }, function () {});
					Logger.system.log("Exited External Process", event);
				} else {
					Logger.system.log("Running External Process", event);
				}
			}
		}, function (payload) {
			var sDisc = {
				finUUID: payload.uuid,
				params: params,
				connected: false,
				target: params.path
			};
			var aWindow = findActiveWindow(sDisc);
			if (aWindow) {
				//Connect the spawned component with an external windwo if found
				sDisc.connected = true;
				aWindow.spawned = sDisc;
			}
			if (ws && ws.readyState === 1) {
				ws.send(JSON.stringify({ topic: "updateWindowList" }));
			}
			Logger.system.log("success", payload.uuid);
			self.spawnedWindows.push(sDisc);
			self.RouterClient.transmit(params.name + ".onSpawned", {
				name: params.name,
				external: true,
				spawnedFromApp: true
			});
			...
			Logger.system.log("Error:", error);
			return cb(error);
		});
	}

	function spawnByAlias(params, foundAsset, cb) {
		fin.desktop.System.launchExternalProcess({
			alias: params.alias,
			lifetime: "window",
			listener: function (event) {
			...
			//A wrapper for the spawned component
			var sDisc = {
				finUUID: payload.uuid,
				params: params,
				connected: false,
...
AssimilationService.prototype = new baseService("assimilationService");
var serviceInstance = new AssimilationService();
serviceInstance.addNeededServices(["authenticationService", "dockingService", "storageService"]);

serviceInstance.onBaseServiceReady(function (callback) {
	Logger.start();
	Logger.system.debug("onBaseServiceReady called");
...
module.exports = serviceInstance;
 ...
 ;(function register() { /* react-hot-loader/webpack */ if (process.env.NODE_ENV !== 'production') { if (typeof __REACT_HOT_LOADER__ === 'undefined') { return; } /* eslint-disable camelcase, no-undef */ var webpackExports = typeof __webpack_exports__ !== 'undefined' ? __webpack_exports__ : module.exports; /* eslint-enable camelcase, no-undef */ if (typeof webpackExports === 'function') { __REACT_HOT_LOADER__.register(webpackExports, 'module.exports', "E:\\chartiq\\finsemble\\src\\services\\assimilation\\assimilationService.js"); return; } /* eslint-disable no-restricted-syntax */ for (var key in webpackExports) { /* eslint-enable no-restricted-syntax */ if (!Object.prototype.hasOwnProperty.call(webpackExports, key)) { continue; } var namedExport = void 0; try { namedExport = webpackExports[key]; } catch (err) { continue; } __REACT_HOT_LOADER__.register(namedExport, key, "E:\\chartiq\\finsemble\\src\\services\\assimilation\\assimilationService.js"); } } })();
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
/* WEBPACK VAR INJECTION */(function(process) {var externalWindow = function () {
	var self = this;
	...
	RouterClient.addListener("LauncherService.shutdownRequest", self.shutdown);

	this.setScalingFactor = function (scalingFactor) {
		self.scalingFactor = scalingFactor;
	};
	this.scaleOut = function (location) {
		if (self.scalingFactor === 1) {
			return location;
		}
		location.left = Math.round(Number(location.left) * self.scalingFactor);
		location.right = Math.round(Number(location.right) * self.scalingFactor);
		location.top = Math.round(Number(location.top) * self.scalingFactor);
		location.bottom = Math.round(Number(location.bottom) * self.scalingFactor);
		location.width = location.right - location.left;
		location.height = location.bottom - location.top;
		return location;
	};
	this.scaledLocation = false;
	this.scaleDown = function (params) {
		if (self.scalingFactor === 1) {
			params.width = params.right - params.left;
			params.height = params.bottom - params.top;
			return params;
		}
		this.scaledLocation = true;
		params.left = Math.round(Number(params.left) / self.scalingFactor);
		params.right = Math.round(Number(params.right) / self.scalingFactor);
		params.top = Math.round(Number(params.top) / self.scalingFactor);
		params.bottom = Math.round(Number(params.bottom) / self.scalingFactor);
		params.width = params.right - params.left;
		params.height = params.bottom - params.top;
		params.x = Math.round(Number(params.x) / self.scalingFactor);
		params.y = Math.round(Number(params.y) / self.scalingFactor);
		return params;
	};
	this.setStartBounds = function (params) {

		params = this.scaleDown(params);
		//	console.log(params);
		self.startBounds = {
			"left": params.left,
			"top": params.top,
	...
	this.setMouseOffset = function (params) {
		self.mouseOffset = {
			left: params.left >= 0 ? params.x - params.left : params.x - params.left,
	...
	this.calculateWindowLocation = function (newMouseLocation) {
		...
		newMouseLocation.x = Math.round(Number(newMouseLocation.x) / self.scalingFactor);
		newMouseLocation.y = Math.round(Number(newMouseLocation.y) / self.scalingFactor);
		if (!self.edge) {
			self.edge = self.GetResizeEdge(self.startBounds, newMouseLocation, self.resizeType);
		}
		var windowBounds = {
		...
		if (!self.edge) {
			...
			console.log("change type should be 1", self);
			changeType = 1;
			switch (self.edge) {
				case "left":
...
module.exports = externalWindow;
 ...
 ;(function register() { /* react-hot-loader/webpack */ if (process.env.NODE_ENV !== 'production') { if (typeof __REACT_HOT_LOADER__ === 'undefined') { return; } /* eslint-disable camelcase, no-undef */ var webpackExports = typeof __webpack_exports__ !== 'undefined' ? __webpack_exports__ : module.exports; /* eslint-enable camelcase, no-undef */ if (typeof webpackExports === 'function') { __REACT_HOT_LOADER__.register(webpackExports, 'module.exports', "E:\\chartiq\\finsemble\\src\\services\\assimilation\\windowModel.js"); return; } /* eslint-disable no-restricted-syntax */ for (var key in webpackExports) { /* eslint-enable no-restricted-syntax */ if (!Object.prototype.hasOwnProperty.call(webpackExports, key)) { continue; } var namedExport = void 0; try { namedExport = webpackExports[key]; } catch (err) { continue; } __REACT_HOT_LOADER__.register(namedExport, key, "E:\\chartiq\\finsemble\\src\\services\\assimilation\\windowModel.js"); } } })();
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
