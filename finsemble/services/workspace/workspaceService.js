...
var util = __webpack_require__(2);
var primaryMonitorDimensions = null;
var DefaultWorkspaces = __webpack_require__(48);
var ConfigClient = __webpack_require__(17);

var Logger = __webpack_require__(1);

Logger.system.debug("Starting Workspace Service");
 ...
 */
var workspaces = ["activeWorkspace", "WS1", "WS2"];
var activeWorkspace = {
	name: "savedWorkspace1",
	windows: ["window 1", "window 2"],
	isDirty: false,
	groups: {}
};
var activeWorkspaceWindow1 = ["container 1"];
var activeWorkspaceWindow1Container1 = {
	id: "container 1",
	componentState: {
		layout: "chartlayout",
		symbol: "aapl"
...
function WorkspaceService() {
	baseService.call(this, arguments);
  ...
  */
	this.defaults = {
		components: [{ type: "Advanced Chart", options: { defaultTop: 300, defaultLeft: 300 } }, { type: "Simple Chart", options: { defaultTop: 330, defaultLeft: 330 } }, { type: "News", options: { defaultTop: 360, defaultLeft: 360 } }, { type: "Insider Trades", options: { defaultTop: 390, defaultLeft: 390 } }, { type: "Financials", options: { defaultTop: 420, defaultLeft: 420 } }, { type: "Company", options: { defaultTop: 300, defaultLeft: 260 } }, { type: "Advanced Chart", options: { defaultTop: 300, defaultLeft: 300, customData: { symbol: "KO" } } }, { type: "Advanced Chart", options: { defaultTop: 270, defaultLeft: 270, customData: { symbol: "MSFT" } } }, { type: "Advanced Chart", options: { defaultTop: 240, defaultLeft: 240, customData: { symbol: "AMZN" } } }, { type: "Advanced Chart", options: { defaultTop: 210, defaultLeft: 210, customData: { symbol: "GOOG" } } }, { type: "Advanced Chart", options: { defaultTop: 180, defaultLeft: 180, customData: { symbol: "IBM" } } }, { type: "Advanced Chart", options: { defaultTop: 150, defaultLeft: 150, customData: { symbol: "FB" } } }, { type: "Advanced Chart", options: { defaultTop: 120, defaultLeft: 120, customData: { symbol: "NFLX" } } }],
		workspace: { name: "default Workspace", windows: [] }
	};
  ...
  * @private
  */
	this.persistDefaultWorkspace = function () {
		Logger.system.debug("Persisting the default workspace");
		...
		//var workspace = self.defaults.workspace;
		DefaultWorkspaces.workspaces.map(function (workspace, index) {
			workspace.components.map(function (item, i) {
				...
				var defaultWindowHash = util.camelCase(workspace.name, item.type);
				...
				Logger.system.debug(customData);
				self.storage.save({
					topic: "finsemble",
			...
			Logger.system.debug("workspace.name:::" + workspace.name);
			self.storage.save({ topic: "finsemble", key: util.camelCase(workspace.name), value: workspace });
		});
	};

	/**
  * Begins the Application startup process.
  * @private
  */
	this.startApp = function () {

		util.getMonitorInfo().then(function (monitorInfo) {
			// @TODO, we can probably get rid of this now
			self.primaryMonitorDimensions = monitorInfo.primaryMonitor.availableRect;
			self.primaryMonitorDimensions.width = self.primaryMonitorDimensions.right - self.primaryMonitorDimensions.left;
			self.primaryMonitorDimensions.height = self.primaryMonitorDimensions.bottom - self.primaryMonitorDimensions.top;
	...
	/**
  * Tells all toolbars to close. Usually called when a monitor is added/removed.
  * @private
  */
	this.closeToolbars = function () {
		Logger.system.debug("Telling toolbars to close");
		RouterClient.transmit("LauncherService.closeToolbars");
	};
	/**
  * Spawns a toolbar on every monitor.
  * @private
  */
	this.spawnToolbars = function () {
		Logger.system.debug("Spawning toolbars");
		var monitorInfo = self.monitorInfo;
		var monitors = [monitorInfo.primaryMonitor].concat(monitorInfo.nonPrimaryMonitors);
		self.availableMonitors = monitors.length;
		var monitorNum = 0;
		monitors.forEach(function (monitor, index) {

			/*			var dimensions = monitor.availableRect;
   			dimensions.left = dimensions.left;
   			dimensions.top = dimensions.top;
   			dimensions.width = dimensions.right - dimensions.left;
   			//@todo get launcher height some other way. this is possibly dumb.
   			dimensions.height = dimensions.bottom - dimensions.top - 40;
   			Logger.system.debug('MonitorDimensions ' + JSON.stringify(dimensions));
   			//spawn will shift the toolbar to the appropriate monitor because we pass monitorDimensions in as customData.
   			var options = {
   				defaultTop: dimensions.top,
   				defaultLeft: dimensions.left,
   				defaultWidth: dimensions.width,
   				customData: {
   					monitorDimensions: dimensions
   				}
   			};*/
			LauncherClient.spawn("Toolbar", { monitor: monitorNum });
			monitorNum++;
		});
	};

	/**
  * Spawns menus that exist outside of the workspace. Similar to dialogs, they spawn at app initialization and then move to wherever the application tells them to move.
  * @private
  */
	this.spawnMenus = function () {
		Logger.system.debug("Spawning menus");
		LauncherClient.spawn("App Launcher");
		LauncherClient.spawn("File Menu");
		LauncherClient.spawn("Workspace Management Menu");
		LauncherClient.spawn("Workspace Overflow Menu");
	};
  ...
  */
	this.initialize = function (params, callback) {
		...
		var tasks = {
			getDefaultWorkspace: function (done) {
				Logger.system.debug("initialize sequence: getDefaultWorkspace");
				//check to see if the app has specified defaultWorkspaces in finsemble config
				ConfigClient.get({ field: "finsemble" }, function (err, finsemble) {
					if (finsemble.workspaces) {
						DefaultWorkspaces.workspaces = finsemble.workspaces;
					}
					done();
				});
			},
			getMonitorDimensions: function (done) {
				Logger.system.debug("initialize sequence: getMonitorDimensions");
				util.getMonitorDimensions().then(function (dimensions) {
					primaryMonitorDimensions = dimensions;
					done();
				});
			},
			getAllworkspaces: function (done) {
				Logger.system.debug("initialize sequence: getAllworkspaces");
				//Get all workspaces
				self.storage.get({ topic: "finsemble", key: "fsblWorkspaces" }, function (err, response) {
					if (err) {
						return done(err);
					}
					if (!response) {
			...
			getActiveWorkspace: ["getDefaultWorkspace", "getAllworkspaces", function (data, done) {
				Logger.system.debug("initialize sequence: getActiveWorkspace");
				self.storage.get({ topic: "finsemble", key: "activeWorkspace" }, function (err, response) {
					if (err) {
						Logger.system.error(err);

						return done(err);
					}
					Logger.system.debug("IN THE WORKSPACES CODE", response);
					if (response) {
						self.activeWorkspace = merge({}, response);
						return self.loadActiveWorkspace(response, done);

						//return done();
					}
					//if we have workspaces but no active load the first workspace.
					...
					if (DefaultWorkspaces.workspaces[0]) {
						//cloning here so that if we change the activeWorkspace's name (e.g., on a saveAs), it won't be reflected in the self.workspaces object.
						self.activeWorkspace = merge({}, DefaultWorkspaces.workspaces[0]);
					}
					self.workspaces = DefaultWorkspaces.workspaces;
					//var windowsRemaining = self.defaults.components.length;
					self.persistDefaultWorkspace();
					self.save();

					DefaultWorkspaces.workspaces[0].components.map(function (item, i) {
						Logger.system.debug("spawning item", item);
						var windowHash = util.camelCase(DefaultWorkspaces.workspaces[0].name, item.type);
						...
						self.spawnWindow(item.type, spawnParams);
					});
		...
		async.auto(tasks, function (err, responses) {
			if (err) {
				Logger.system.error(err);
			}
			Logger.system.debug("--all done");
			self.update("workspace:load");
			self.update("workspace:initialization");
			return callback();
		});
	};
	...
	/**
  *
  * Saves active workspace to storage. Upon completion, it broadcasts an update on the `Finsemble.WorkspaceService.update` channel.
  */
	this.save = function (cb) {
		Logger.system.debug("Saving the active workspace.");
		if (!cb) {
			cb = function noop() {};
		}
		function saveWorkspaces(done) {
			self.storage.save({ topic: "finsemble", key: "fsblWorkspaces", value: self.workspaces }, done);
		}
		function saveActiveWorkspace(done) {
			self.storage.save({ topic: "finsemble", key: "activeWorkspace", value: self.activeWorkspace }, done);
		}
		async.parallel([saveWorkspaces, saveActiveWorkspace], cb);
	};
	/**
  * Removes window from active workspace.
  * @param {object} params
  ...
  */
	this.removeWindow = function (params, cb) {
		Logger.system.debug("Removing Window " + params.name + " from the active workspace.");
		self.activeWorkspace.isDirty = true;
		var windowName = params.name;
		var windex = self.activeWorkspace.windows.indexOf(windowName);
		if (windex > -1) {
			self.activeWorkspace.windows.splice(windex, 1);
		} else {
			//someone tried to remove a window that wasn't in the workspace. still let they get their cb.
			if (cb) {
		...
		//Delete data for the window and any containers.
		self.storage.get({ topic: "finsemble", key: windowHash }, function (err, response) {
			if (!response /*|| !response.data*/) {
					return cb();
				} //@TODO, Terry, this was throwing an error. Looks like response.data is wrong here?
			var data = response;
			data.containers = [windowName];
			if (data && data.containers) {
				data.containers.forEach(function (container) {
					var containerHash = util.camelCase("activeWorkspace", windowName, container);
					self.storage.delete({ topic: "finsemble", key: containerHash });
				});
			}
			self.storage.delete({ topic: "finsemble", key: windowHash });

			if (cb) {
				self.update("window:remove");
				cb();
			}
		});
	};
  ...
  */
	this.addWindow = function (params) {
		Logger.system.debug("Adding window " + params.name + " to the active workspace.");
  ...
  * @returns {Promise}
  */
	this.getWindowData = function (windowHash) {
		return new Promise(function (resolve, reject) {
			self.getData(windowHash).then(function (windowData) {
				resolve(windowData);
			});
		});
	};
	this.workspaceIsAlreadySaved = function (workspaceName) {
		let workspaceIndex = self.workspaces.findIndex(workspace => workspace.name === workspaceName);
  ...
  */
	this.saveAs = function (params, callback) {
		Logger.system.debug("Saving the active workspace with the name: " + params.name);
		//@todo, make sure name comes in as a param.
		var workspaceName = params.name;
		var workspaceIsAlreadySaved = this.workspaceIsAlreadySaved(workspaceName);
		var savedWorkspaceIndex = null;
		var newWorkspace = {
			name: workspaceName,
			windows: [],
			isDirty: false,
			groups: self.activeWorkspace.groups
		};

		//copy activeWorkspace stuff.
		Logger.system.debug("Copying activeWorkspace to " + "\"" + workspaceName + "\"");
		var windowsToSave = self.activeWorkspace.windows;
		var windowsRemaining = windowsToSave.length;
		if (windowsRemaining === 0) {
			invokeCallback();
		}

		if (workspaceIsAlreadySaved) {
			savedWorkspaceIndex = self.workspaces.findIndex(workspace => workspace.name === workspaceName);
			getWorkspaceData(self.activeWorkspace.name).then(function (workspaceData) {
				/**
     ...
     */
				if (workspaceData && workspaceData.windows) {
					let windowsToDelete = workspaceData.windows.filter(windowName => !self.activeWorkspace.windows.includes(windowName));
					windowsToDelete.forEach(windowName => {
						let windowHash = util.camelCase(self.activeWorkspace.name, windowName);
						self.storage.delete({ topic: "finsemble", key: windowHash });
					});
				}
		...
		function decrementWindowCount() {
			windowsRemaining--;
			Logger.system.debug("DECREMENTING WINDOW COUNT", windowsRemaining, "REMAINING");
		...
		function invokeCallback() {
			...
			self.update("Workspace:Save As");
			//SaveAs, then persist workspaceObject and activeWorkspace.
			self.storage.save({ topic: "finsemble", key: util.camelCase(workspaceName), value: newWorkspace }, function () {
				self.save(callback);
			});
		}
		function finishSave() {
			windowsToSave.forEach(function (windowName) {
				Logger.system.debug("saving window..", windowName);
				var oldWindowHash = util.camelCase("activeWorkspace", windowName);
				var newWindowHash = util.camelCase(workspaceName, windowName);
				var windowData;
				newWorkspace.windows.push(windowName);
				//save after adding window.
				self.storage.save({ topic: "finsemble", key: util.camelCase(workspaceName), value: newWorkspace });
				self.getWindowData(oldWindowHash).then(function (windowData) {
					if (!windowData) {
						decrementWindowCount();
						Logger.system.warn("No window data found for " + "'" + oldWindowHash + "\"" + "; continuing to the next window.");
						return;
					}
					self.storage.save({ topic: "finsemble", key: newWindowHash, value: windowData });

					// if (!windowData.containers) {
					// 	decrementWindowCount();

					// 	Logger.system.warn('No containers found for ' + "'" + oldWindowHash + '"' +
					// 		'; continuing to the next window.');
					// 	return;
					// }
					[windowData.name].forEach(function (containerName) {
						//@todo, refactor to use windowRemaining and containerRemaining like load.
						var oldContainerHash = util.camelCase("activeWorkspace", windowName, containerName);
						var newContainerHash = util.camelCase(workspaceName, windowName, containerName);
						self.getData(oldContainerHash).then(function (containerData) {
							if (!containerData) {
								decrementWindowCount();
								return;
							}
							self.storage.save({ topic: "finsemble", key: newContainerHash, value: containerData });
							decrementWindowCount();
						});
					});
				});
	...
	/**
  * Helper to retrieve a key from storage.
  * @private
  ...
  */
	this.getData = function (key) {
		return new Promise(function (resolve, reject) {
			self.storage.get({ topic: "finsemble", key: key }, function (err, response) {
				var data;
				if (response) {
					data = response;
				}
  ...
  */
	this.loadActiveWorkspace = function (activeWorkspace, callback) {
		if (!activeWorkspace || !activeWorkspace.windows || activeWorkspace.windows.length === 0) {
			self.update("workspace:load");
			callback();
			return;
		}
		Logger.system.debug("load activeworkspace", activeWorkspace);
		tasks = [function getWindows(done) {
			getAllWorkspaceWindows({ name: "activeWorkspace", windows: activeWorkspace.windows }, done);
		}, function loadEachWindow(windowInfo, done) {
			spawnWindowList(windowInfo, done);
		}];
		...
		//self.storage.setStore({ topic: "finsemble", dataStore: "localStorage" });
		self.storage.keys({ topic: "finsemble", keyPrefix: "active" });
	};
	function getAllWorkspaceWindows(workspace, callback) {
		async.map(workspace.windows, function (win, done) {
			var newWindowHash = util.camelCase(workspace.name, win);
			self.storage.get({ topic: "finsemble", key: newWindowHash }, function (err, response) {
				if (err) {
					return done(err);
				}
				if (!response) {
	...
	function spawnWindowList(windowList, callback) {
		async.each(windowList, function (win, done) {
			if (!win) {
				return done();
			}
			if (!win.customData.component.inject) {
				// @todo - remove when OpenFin Fixes preload
				self.spawnWindow(win.customData.component.type, {
					options: win
				});
			}
			done();
	...
	/**
  * Helper function that retrieves workspace data from storage.
  * @private
  * @param {string} workspaceName
  * @returns {Promise} Promise that resolves with workspaceData.
  */
	function getWorkspaceData(workspaceName) {
		//camelCase in case the display name has spaces in it.
		return self.getData(util.camelCase(workspaceName));
	}
	/**
  * Loads a new or previously saved workspace.
  * @param {object} 		params
  ...
  * @param {string} 		[params.name] Workspace Name
  */
	this.load = function (params, message) {
		if (!params.workspace && !params.name) {
			throw new Error("To load a workspace, please provide the new workspace, or the name of a previously saved workspace.");
		}
		//@todokill open windows.
		var workspaceName = params.name;
		Logger.system.debug("Loading " + "\"" + workspaceName + "\"");
   ...
   * @returns {Promise}
   */
		function getWindowDescriptor(hash) {
			return new Promise(function (resolve, reject) {
				var windowDescriptor;
				self.getData(hash).then(function (data) {
					resolve(data);
				});
			});
		}
   ...
   * @returns {Promise}
   */
		function copyWindowData(windowDescriptor) {
			return new Promise(function (resolve, reject) {
				var newWindowHash = util.camelCase("activeWorkspace", windowDescriptor.name);
				self.storage.save({ topic: "finsemble", key: newWindowHash, value: windowDescriptor });
				resolve();
			});
		}

		/**
   * Helper function that retrieves copys the containers in a windowDescritpor to the activeWorkspace in storage
   * @private
   * @param {LauncherClient~windowDescriptor} windowDescriptor
   * @returns {Promise} Promise that resolves with workspaceData.
   */
		function copyContainers(windowDescriptor) {
			var windowName = windowDescriptor.name;
			var containerNames = [windowDescriptor.name];
			return new Promise(function (resolve, reject) {
				if (!containerNames) {
					resolve();
					return;
				}
				var containersRemaining = containerNames.length;
				//return if there's nothing to copy.
				let workspaceLoadTimeout = setTimeout(function () {
					//called if it takes very long for the code to complete.
					var errorString = "Timeout in WorkspaceService.load => CopyContainers";
					Logger.system.error(errorString);
					reject(new Error(errorString));
				}, 2000 * containerNames.length);
				if (containerNames.length) {
					containerNames.forEach(function (containerName) {
						var containerHash = util.camelCase(workspaceName, windowName, containerName);
						self.getData(containerHash).then(function (containerData) {
							//decrement value. Even if no data was returned, we tried.
							containersRemaining--;
							//only proceed if data was returned.
							if (!containerData) {
								if (containersRemaining === 0) {
									clearTimeout(workspaceLoadTimeout);
									resolve();
								}
								return;
							}
							var newContainerHash = util.camelCase("activeWorkspace", windowName, containerName);
							self.storage.save({ topic: "finsemble", key: newContainerHash, value: containerData });
							if (containersRemaining === 0) {
								clearTimeout(workspaceLoadTimeout);
					...
					Logger.system.warn("No containers found for " + "\"" + windowName + "\"");
					resolve();
				}
			});
		}
		var workspaceIsAlreadySaved = false;
		//used to overwrite.
		var savedWorkspaceIndex = -1;
		for (var i = 0; i < self.workspaces.length; i++) {
			if (workspaceName === self.workspaces[i].name) {
				workspaceIsAlreadySaved = true;
		...
		function loadComplete() {
			self.save();
			Logger.system.debug("All windows loaded for " + "\"" + workspaceName + "\"");
			if (message) {
				self.sendResponse(message, self.activeWorkspace);
			}
			self.update("workspace:load");
			self.isLoading = false;
			self.processWorkspaceLoadQueue();
		}
		getWorkspaceData(workspaceName).then(function (workspaceDescriptor) {
			Logger.system.info("-------->DESCRIPTOR", workspaceDescriptor);
			if (!workspaceIsAlreadySaved) {
				let activeWorkspaceClone = merge({}, self.activeWorkspace);
				self.workspaces.push(activeWorkspaceClone);
			}
			//update activeWorkspace even if we're loading a workspace with no windows.
			if (!workspaceDescriptor) {
				Logger.system.info("No workspaceDescriptor found for " + "\"" + workspaceName + "\"" + ". Loading blank workspace");
				return loadComplete();
			}
			workspaceDescriptor.initializedWindows = [];

			var windowsToLoad = workspaceDescriptor.windows;
			if (!windowsToLoad) {
				Logger.system.warn("No windows found for workspace " + "\"" + workspaceName + "\"");
				return loadComplete();
			}
			...
			self.activeWorkspace.windows = windowsToLoad;
			self.activeWorkspace.isDirty = false;
			self.activeWorkspace.groups = workspaceDescriptor.groups;

			Logger.system.debug("WorkspaceDescritpr found for " + "\"" + workspaceName + "\"" + ". Need to load " + workspaceDescriptor.windows.length + " windows");

			if (windowsToLoad.length === 0) {
				return loadComplete();
			}

			async.each(windowsToLoad, loadWindow, loadComplete);
			function loadWindow(windowName, done) {
				var windowHash = util.camelCase(workspaceName, windowName);
				getWindowDescriptor(windowHash).then(function (windowDescriptor) {
					if (!windowDescriptor) {
						return done();
					} // workspace must have gotten into a bad state. Let's not bomb out.
					copyWindowData(windowDescriptor).then(function () {
						copyContainers(windowDescriptor).then(function () {
							Logger.system.debug(`Containers copied for ${windowDescriptor.name}. About to spawn it.`);
							self.spawnWindow(windowDescriptor.customData.component.type, { options: windowDescriptor }).then(function () {
								Logger.system.debug(`${windowDescriptor.name} spawned.`);
								done();
							}).catch(function (err) {
								Logger.system.error("Error in spawn", err);
								done();
							});
						});
					});
	...
	/**
  * @param {LauncherClient~windowDescriptor} windowDescriptor Object to derive window properties from.
  * @private
  * @returns {Promise}
  */
	this.spawnWindow = function (component, windowDescriptor) {
		function componentReadyCallback(err, message) {
			var windowName = message.data.name;
			Logger.system.debug("includes--", self.activeWorkspace);
			if (self.activeWorkspace.windows.includes(windowName)) {
				self.activeWorkspace.initializedWindows.push(windowName);
			}
			RouterClient.removeListener(windowDescriptor.name + ".componentReady", componentReadyCallback);
		}
		RouterClient.addListener(windowDescriptor.name + ".componentReady", componentReadyCallback);

		return new Promise(function (resolve, reject) {
			Logger.system.debug("spawning", component, windowDescriptor);
			LauncherClient.spawn(component, windowDescriptor, function (err, response) {
				if (err) {
					Logger.system.error("Workspace.spawnWindow" + new Error(err));
					return reject(err);
				}
				resolve();
			});
  ...
  */
	this.update = function (reason) {
		Logger.system.debug("Finsemble.WorkspaceService.update");
		RouterClient.publish("Finsemble.WorkspaceService.update", {
			workspaces: self.workspaces,
			activeWorkspace: self.activeWorkspace,
			isDirty: self.activeWorkspace.isDirty,
			groups: self.activeWorkspace.groups,
			reason: reason
		});
	};
	this.getGroupData = function (message) {
		message.sendQueryResponse(self.activeWorkspace.groups);
	};
	this.saveGroupData = function (params, cb) {
		var data = params.data,
		    workspaceName = params.workspaceName;
		if (!workspaceName) {
			workspaceName = "activeWorkspace";
		}
		key = workspaceName + "DockingGroups";
		self.activeWorkspace.groups = data;
	};
	/**
  ...
  */
	this.remove = function (params, cb) {
		if (!params.workspace && !params.name) {
			throw new Error("To remove a workspace, please provide a reference to it, or its name.");
		}
		var workspaceName = params.name || params.workspace.name;
		var wsIndex = -1,
		    windowsToRemove = 0;
		for (var i = 0; i < self.workspaces.length; i++) {
			var ws = self.workspaces[i];
			if (ws.name === workspaceName) {
				wsIndex = i;
				break;
			}
		}
		Logger.system.debug("Removing " + "\"" + workspaceName + "\"");
		if (wsIndex > -1) {
			self.workspaces.splice(wsIndex, 1);
		} else {
			Logger.system.error("Workspace " + "\"" + workspaceName + "\"" + " not found.");
			if (cb) {
				self.update("Workspace:remove");
				cb();
			}
			return;
		}
		...
		//remove windows
		self.getData(workspaceName).then(function (workspaceDescriptor) {
			self.storage.delete({ topic: "finsemble", key: workspaceName });
			...
			workspaceDescriptor.windows.forEach(function (windowName) {
				var windowHash = util.camelCase(workspaceName, windowName);
				//getting the WindowDescriptor, which contains an array of its containers.
				self.getData(windowHash).then(function (windowDescriptor) {
					self.storage.delete({ topic: "finsemble", key: windowHash });
					if (!windowDescriptor) {
						decrementWindowCount();
						return;
					}
					[windowDescriptor.name].forEach(function (containerName) {
		...
		function decrementWindowCount() {
			windowsToRemove--;
			Logger.system.debug("DECREMENTING WINDOW COUNT", windowsToRemove, "remaining");
  ...
  * Clones a workspace.
  * @param {object} params
  * @param {string} params.workspaceName Name of workspace you'd like to clone.
  you rename.
  * @param {function} callback Callback
  */
	this.clone = function (params, callback) {
		var newName = params.workspaceName + " ";
		this.rename({
			oldName: params.workspaceName,
			newName: newName,
  ...
  */
	this.rename = function (params, callback) {
		//Don't bother if someone just clicked the button without changing it.
		...
		if (params.removeOldWorkspace === false) {
			...
			for (var i = 0; i < self.workspaces.length; i++) {
				var wsNameStripped = new String(self.workspaces[i].name).valueOf();
				wsNameStripped = wsNameStripped.replace(/\s\(\d?.*\)/, "").trim();

				if (self.workspaces[i].name.indexOf(params.newName) > -1 || newNameStripped.indexOf(wsNameStripped) > -1) {
					duplicateCount++;
				}
			}
			if (duplicateCount) {
		...
		Logger.system.debug("Renaming " + "'" + params.oldName + "'");
		...
		if (params.removeOldWorkspace && self.activeWorkspace.name === params.oldName) {
			return self.remove({
				name: params.oldName
			}, function (err, response) {
				self.saveAs({
					...
					self.activeWorkspace.name = params.newName;
					self.update("rename");
					if (callback) {
						callback();
					}
		...
		function finishRename() {
			...
			var weAreOverwriting = self.workspaceIsAlreadySaved(params.newName);

			for (var i = 0; i < self.workspaces.length; i++) {
				if (self.workspaces[i].name === params.oldName) {
					oldWorkspaceIndex = i;
					break;
				}
			}
			...
			self.storage.save({ topic: "finsemble", key: util.camelCase(params.newName), value: newWorkspace });
			if (params.removeOldWorkspace) {
				self.workspaces.splice(oldWorkspaceIndex, 0, newWorkspace);
				self.remove({
					name: params.oldName
				});
			} else {
				self.workspaces.splice(oldWorkspaceIndex + 1, 0, newWorkspace);
			}
		...
		self.getData(util.camelCase(params.oldName)).then(function (workspaceDescriptor) {

			if (!workspaceDescriptor || !workspaceDescriptor.windows) {
				//@todo, this case would be a crucial error. A workspace was saved without windows??? wat.
				Logger.system.error(new Error("The workspace Descriptor either does not exist or there are no windows."));
				finishRename();
				return;
			}
    ...
    */
			...
			function copyWindow(params) {
				...
				return new Promise(function (resolve, reject) {
					...
					Logger.system.debug("newWorkspace.windows", newWorkspace.windows);
    ...
    * @returns {Promise}
    */
			function copyContainers(params) {
				var oldWindowHash = params.oldWindowHash,
				    newWindowHash = params.newWindowHash,
				    windowDescriptor = params.windowDescriptor,
				...
				return new Promise(function (resolve, reject) {
					...
					[windowDescriptor.name].forEach(function (containerName) {
						var oldContainerHash = util.camelCase(oldWindowHash, containerName);
						var newContainerHash = util.camelCase(newWindowHash, containerName);
						self.getData(oldContainerHash, function (containerData) {
							containersRemaining--;
							if (!containerData) {
								decrementWindowCount();
								return;
			...
			copyWindows(workspaceDescriptor.windows).then(function () {
				//this is so we can splice the renamed ws back into the proper order.
				finishRename();
			});
		}).catch(function (err) {
			callback(err, null);
	...
	this.workspaceLoadQueue = [];
	this.queueWorkspaceLoad = function (params, message) {
		this.workspaceLoadQueue.push({ params, message });
	};
	this.processWorkspaceLoadQueue = function () {
		if (this.workspaceLoadQueue.length) {
  ...
  */
	this.switchTo = function (params, message) {
		if (this.isLoading) {
			this.queueWorkspaceLoad(params, message);
		} else {
			this.isLoading = true;
			RouterClient.transmit("Launcher.resetSpawnStagger", {});
			Logger.system.debug("Switching workspace to " + "\"" + params.name + "\"");
			if (self.activeWorkspace && self.activeWorkspace.windows) {
				var activeWindows = merge({}, self.activeWorkspace.windows);
				LauncherClient.getActiveDescriptors((err, descriptors) => {
					if (err) {
						message.sendQueryResponse(err, null);
						return;
					}
					let workspaceDescriptors = [];
					activeWindows.forEach(function (windowName) {
						let descriptor = descriptors[windowName];
						if (!descriptor) {
							return;
						}
						workspaceDescriptors.push(descriptor);
					});
					async.each(workspaceDescriptors, closeWindow, function () {
						self.load({ name: params.name }, message);
					});
				});
			}
		}
		function closeWindow(descriptor, done) {
			//Iterate through windows; if it's initialized, send it a message and tell it to close itself. If not, force close it.
			let windowName = descriptor.name;
			self.storage.delete({ topic: "finsemble", key: util.camelCase("activeWorkspace", windowName) });
			//@todo this is for 'containers'. By default they're the same name as the window. This is where component state is stored.
			self.storage.delete({ topic: "finsemble", key: util.camelCase("activeWorkspace", windowName, windowName) });
			Logger.system.debug("before getFinWindow", descriptor);
			descriptor.windowName = descriptor.name;
			util.getFinWindow(descriptor).then(finWindow => {
				function onWindowClosed() {
					finWindow.removeEventListener("closed", onWindowClosed);
					done();
				}
				finWindow.addEventListener("closed", onWindowClosed);
				Logger.system.debug("FinWindow found", descriptor.name);
				if (self.activeWorkspace.initializedWindows && self.activeWorkspace.initializedWindows.includes(windowName)) {
					RouterClient.transmit("WorkspaceService." + windowName, {
						command: "close"
					});
				} else {
					//False is to allow any window that is listening for close-requested to do any cleanup work.
					finWindow.close(false, function () {
						Logger.system.debug("successfully closed", windowName);
					}, function (err) {
						Logger.system.error(err);
					});
				}
			}).catch(function () {
				Logger.system.error("Finwindow not found for descriptor:", descriptor);
				done();
			});
		}
	};
  ...
  * @param {object} data
  */
	this.sendResponse = function (message, data) {
		var err = null;
  ...
  */
	this.bringWindowsToFront = function (params) {
		var windowsToRevive = [];
		/**
   * Goes through all windows and returns the windows that are:
   * 1) In the active workspace and
   * 2) On the monitor that bringToFront was clicked on.
   */
		util.getMonitorFromCommand(params.monitor, params.windowIdentifier, function (monitorInfo) {
			fin.desktop.System.getAllWindows(function (windows) {
				windows.forEach(splinterAgent => {
					splinterAgent.childWindows.forEach(function (win) {
						Logger.system.debug("BringToFront Windows:" + JSON.stringify(self.activeWorkspace));
						if (params.monitor === "all" || util.windowOnMonitor(win, monitorInfo.monitorRect)) {
							windowsToRevive.push({
								name: win.name,
								uuid: win.uuid
							});
						}
				...
				windowsToRevive.forEach(function (finWin) {
					RouterClient.transmit("WorkspaceService." + finWin.name, {
						command: "restore"
					});
					RouterClient.transmit("WorkspaceService." + finWin.name, {
  ...
  */
	this.createRouterEndpoints = function () {
		...
		RouterClient.addResponder("WorkspaceService.save", function (err, message) {
			self.save();
			self.sendResponse(message, { status: "success" });
		});

		RouterClient.addResponder("WorkspaceService.saveAs", function (err, message) {
			self.saveAs(message.data, function () {
				self.sendResponse(message, { status: "success" });
			});
		});

		RouterClient.addResponder("WorkspaceService.bringWindowsToFront", function (err, message) {
			self.bringWindowsToFront(message.data);
		});

		RouterClient.addResponder("WorkspaceService.load", function (err, message) {
			self.load(message.data);
		});

		RouterClient.addResponder("WorkspaceService.add", function (err, message) {
			self.add(message.data);
		});

		RouterClient.addResponder("WorkspaceService.remove", function (err, message) {
			self.remove(message.data, self.sendResponse(message));
		});

		RouterClient.addResponder("WorkspaceService.switchTo", function (err, message) {
			return self.switchTo(message.data, message);
		});

		RouterClient.addResponder("WorkspaceService.organizeWindows", function (err, message) {
			return self.addListener();
		});

		RouterClient.addResponder("WorkspaceService.getWorkspaces", function (err, message) {
			return self.sendResponse(message, self.getWorkspaces());
		});

		RouterClient.addResponder("WorkspaceService.getActiveWorkspace", function (err, message) {
			return self.sendResponse(message, self.getActiveWorkspace());
		});
		RouterClient.addResponder("WorkspaceService.addWindow", function (err, message) {
			self.addWindow(message.data);
			self.save();
			self.sendResponse(message, { status: "finished" });
		});
		RouterClient.addResponder("WorkspaceService.removeWindow", function (err, message) {
			self.removeWindow(message.data, function () {
				self.save();
				self.sendResponse(message, { status: "finished" });
			});
		});
		RouterClient.addResponder("WorkspaceService.rename", function (err, message) {
			if (!message.data.newName) {
				self.sendResponse(message, new Error("No new name provided for rename"), null);
				return;
			}
			self.rename(message.data, function (err, response) {
				self.sendResponse(message, err, response);
			});
		});
		RouterClient.addResponder("WorkspaceService.clone", function (err, message) {
			if (!message.data.name) {
				self.sendResponse(message, new Error("No new name provided for rename"), null);
				return;
			}
			self.clone({
				workspaceName: message.data.name
			}, function (err, response) {
				self.sendResponse(message, response);
			});
		});

		RouterClient.addResponder("WorkspaceService.isActiveWorkspaceDirty", function (err, message) {
			self.sendResponse(message, self.activeWorkspace.isDirty);
		});

		RouterClient.addListener("WorkspaceService.setActiveWorkspaceDirty", function (err, message) {
			self.activeWorkspace.isDirty = true;
			self.update();
			self.save();
		});
		RouterClient.addListener("WorkspaceService.saveGroupData", function (err, message) {
			if (message.data.groupData) {
				self.saveGroupData({
					data: message.data.groupData
				}, function noop() {});
			}
		});
		RouterClient.addResponder("WorkspaceService.getGroupData", function (err, message) {
			self.getGroupData(message);
		});

		RouterClient.subscribe("DockingService.groupUpdate", function (err, message) {
			if (self.activeWorkspace && message.data.workspaceName === self.activeWorkspace.name) {
				Logger.system.debug(message.data, "GROUP UPDATE", self.activeWorkspace.name);

				self.activeWorkspace.groups = message.data.groupData;
				self.save();
			}
		});
	};
...
WorkspaceService.prototype = new baseService();
var serviceInstance = new WorkspaceService("workspaceService");
//wait for launcherService to come onboard before initializing.

serviceInstance.addNeededServices(["dockingService", "launcherService", "storageService", "authenticationService"]);
serviceInstance.addNeededClients(["launcherClient", "storageClient"]);
...
module.exports = serviceInstance;
 ...
 ;(function register() { /* react-hot-loader/webpack */ if (process.env.NODE_ENV !== 'production') { if (typeof __REACT_HOT_LOADER__ === 'undefined') { return; } /* eslint-disable camelcase, no-undef */ var webpackExports = typeof __webpack_exports__ !== 'undefined' ? __webpack_exports__ : module.exports; /* eslint-enable camelcase, no-undef */ if (typeof webpackExports === 'function') { __REACT_HOT_LOADER__.register(webpackExports, 'module.exports', "E:\\chartiq\\finsemble\\src\\services\\workspace\\workspaceService.js"); return; } /* eslint-disable no-restricted-syntax */ for (var key in webpackExports) { /* eslint-enable no-restricted-syntax */ if (!Object.prototype.hasOwnProperty.call(webpackExports, key)) { continue; } var namedExport = void 0; try { namedExport = webpackExports[key]; } catch (err) { continue; } __REACT_HOT_LOADER__.register(namedExport, key, "E:\\chartiq\\finsemble\\src\\services\\workspace\\workspaceService.js"); } } })();
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
module.exports = {"comment":"this file defines the default workspaces for development","workspaces":[{"name":"Workspace 1","initializedWindows":[],"windows":[],"components":[{"type":"Advanced Chart","options":{"top":60,"left":0},"customData":{"component":{"symbol":"AMZN"}}}]},{"name":"Workspace 2","initializedWindows":[],"windows":[],"components":[{"type":"Advanced Chart","options":{"top":270,"left":270}},{"type":"Advanced Chart","options":{"top":300,"left":300},"customData":{"component":{"symbol":"GOOG"}}},{"type":"Simple Chart","options":{"top":330,"left":330}},{"type":"News","options":{"top":360,"left":360}},{"type":"Financials","options":{"top":390,"left":390}},{"type":"Company","options":{"top":420,"left":420}}]}]}
