...
function RouterServiceConstructor() {
	...
	function removeListenerHandler(incomingTransportInfo, channel, origin) {
		...
		if (!removed) {
			Logger.system.warn("failed to remove non-existance listener on " + channel + " -- request from " + origin);
		}
	}
	...
	function queryHandler(incomingTransportInfo, channel, queryID, message) {
		...
		if (channel in respondersMap) {
			...
			Logger.system.debug("query routed to responder", channel, message, { timestamp, navstart, timenow });
	...
	function queryResponseHandler(incomingTransportInfo, queryID, message) {
		if (queryID in queryIDMap) {
			...
			// error no responder defined to get the query
			Logger.system.error("queryResponse could not find queryID " + queryID + " -- response from " + message.header.origin);
		}
	}
	...
	function removeResponderHandler(incomingTransportInfo, channel, origin) {
		if (channel in respondersMap) {
			...
			Logger.system.warn("failed to remove non-existance responder on " + channel + " -- request from " + origin);
		}
	}
	...
	function publishHandler(incomingTransportInfo, topic, message) {
		...
		if (outgoingTransportInfo === null) {
			// error no responder defined to get the query
			Logger.system.warn("failed to publish to non-existance pubsub responder for " + topic + " -- request from " + message.header.origin);
		}
	}
	...
	function notifyHandler(incomingTransportInfo, subscribeID, message) {
		if (subscribeID in subscribersIDMap) {
			...
			Logger.system.error("Notify failed since could not find subscriberID", subscribeID, message);
		}
	}
	...
	function removePubSubResponderHandler(incomingTransportInfo, topic, origin) {
		if (topic in subPubRespondersMap) {
			...
			Logger.system.warn("failed to remove non-existance PubSubResponderr for " + topic + " -- request from " + origin);
		}
	}
	...
	// used by RouterClient to verify initial communication
	...
	function initializeTransports(params, readyCallback) {
		...
		var transportNames = RouterTransport.getActiveTransports();
		var retrieved = 0;
  ...
  */
	this.initialize = function (readyCallBack) {
		ConfigUtil.getExpandedRawManifest(function (manifest) {
			initializeTransports({ routerDomainRoot: manifest.finsemble.moduleRoot, routerSharedWorker: manifest.finsemble.moduleRoot + "/common/routerSharedWorker.js" }, readyCallBack);
		});
	};
	return this;
}
...
serviceInstance.initialize(function () {
	baseService = new baseService("routerService");
	baseService.sendOnlineToParent();
});

window.RouterService = serviceInstance; // make accessible for convenience (private window)

 ;(function register() { /* react-hot-loader/webpack */ if (process.env.NODE_ENV !== 'production') { if (typeof __REACT_HOT_LOADER__ === 'undefined') { return; } /* eslint-disable camelcase, no-undef */ var webpackExports = typeof __webpack_exports__ !== 'undefined' ? __webpack_exports__ : module.exports; /* eslint-enable camelcase, no-undef */ if (typeof webpackExports === 'function') { __REACT_HOT_LOADER__.register(webpackExports, 'module.exports', "E:\\chartiq\\finsemble\\src\\services\\router\\routerService.js"); return; } /* eslint-disable no-restricted-syntax */ for (var key in webpackExports) { /* eslint-enable no-restricted-syntax */ if (!Object.prototype.hasOwnProperty.call(webpackExports, key)) { continue; } var namedExport = void 0; try { namedExport = webpackExports[key]; } catch (err) { continue; } __REACT_HOT_LOADER__.register(namedExport, key, "E:\\chartiq\\finsemble\\src\\services\\router\\routerService.js"); } } })();
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
