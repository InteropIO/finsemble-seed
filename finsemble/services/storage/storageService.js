...
/* WEBPACK VAR INJECTION */(function(process) {//Abstract
...
module.exports = BaseStorage;
 ...
 ;(function register() { /* react-hot-loader/webpack */ if (process.env.NODE_ENV !== 'production') { if (typeof __REACT_HOT_LOADER__ === 'undefined') { return; } /* eslint-disable camelcase, no-undef */ var webpackExports = typeof __webpack_exports__ !== 'undefined' ? __webpack_exports__ : module.exports; /* eslint-enable camelcase, no-undef */ if (typeof webpackExports === 'function') { __REACT_HOT_LOADER__.register(webpackExports, 'module.exports', "E:\\chartiq\\finsemble\\src\\services\\storage\\models\\base.js"); return; } /* eslint-disable no-restricted-syntax */ for (var key in webpackExports) { /* eslint-enable no-restricted-syntax */ if (!Object.prototype.hasOwnProperty.call(webpackExports, key)) { continue; } var namedExport = void 0; try { namedExport = webpackExports[key]; } catch (err) { continue; } __REACT_HOT_LOADER__.register(namedExport, key, "E:\\chartiq\\finsemble\\src\\services\\storage\\models\\base.js"); } } })();
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var BaseStorage = __webpack_require__(29);
...
Logger.system.log("Starting Storage Service");

var Storage = function () {
	baseService.call(this, arguments);
	window.finsemble.storageModels = {};
	LocalStorage = __webpack_require__(60);
	Redis = __webpack_require__(61);
	var self = this;

	var currentDefaultDataStore;
	var mapTopicsToDataStore = {};

	this.storageObjects = {};

	//Load each model into our service.This will remove base models if a user uses the same key
	this.loadModels = function () {
		var models = window.finsemble.storageModels;
		for (var modelKey in models) {
			if (self.storageObjects && !self.storageObjects[modelKey]) {
				...
				models[modelKey].setBaseName(self.parentUuid);
				models[modelKey].setUser("default");
				self.storageObjects[modelKey] = models[modelKey];
			}
		}
	};

	var map = {
		"Storage.setUser": function (err, event) {
			Logger.system.debug("Storage setUser", event);
			self.setUser(event, function (err, data) {
				event.sendQueryResponse(err, data);
			});
		},
		"Storage.setStore": function (err, event) {
			Logger.system.debug("Storage setStore", event);
			self.setStore(event);
			event.sendQueryResponse(null, { status: "success" });
		},
		"Storage.save": function (err, event) {
			Logger.system.debug("Storage save", event);
			self.save(event, function (err, data) {
				event.sendQueryResponse(err, data);
			});
		},
		"Storage.delete": function (err, event) {
			Logger.system.debug("Storage delete", event);
			self.delete(event, function (err, data) {
				event.sendQueryResponse(err, data);
			});
		},
		"Storage.get": function (err, event) {
			Logger.system.debug("Storage get", event);
			self.get(event, function (err, data) {
				event.sendQueryResponse(err, data);
			});
		},
		"Storage.keys": function (err, event) {
			Logger.system.debug("Storage keys", event);
			self.keys(event, function (err, data) {
				event.sendQueryResponse(err, data);
			});
		},
		"Storage.getMultiple": function (err, event) {
			Logger.system.debug("Storage getMultiple", event);
			event.sendQueryResponse(err, "get multiple is not implemented");
		},
		"Storage.clearCache": function (err, event) {
			Logger.system.debug("Storage clearCache", event);
			self.clearCache(event, function (err, data) {
				event.sendQueryResponse(err, data);
			});
		}
	};
	...
	this.loadListeners = function () {
		for (item in map) {
			RouterClient.addResponder(item, map[item]);
		}
	};

	this.setUser = function (event, cb) {
		for (var modelKey in self.storageObjects) {
			self.storageObjects[modelKey].setUser(event.data.user);
			Logger.system.log("Model: " + modelKey + " User: " + JSON.stringify(event.data.user));
		}
		return cb(null, { status: "success" });
	};

	// if topic defined set store for topic; otherwise set the default data store
	this.setStore = function (event) {
		if ("topic" in event) {
			Logger.system.debug("setStore for topic", event);
			mapTopicsToDataStore[event.topic] = event.dataStore;
		} else {
			Logger.system.debug("setStore for default", event);
			currentDefaultDataStore = event.dataStore;
		}
	};

	// return topic store if previously defined with setStore(); otherwise, return default datastore
	this.getStore = function (event) {
		var dataStore = currentDefaultDataStore;
		if (event.topic !== undefined) {
			var topicDataStore = mapTopicsToDataStore[event.topic];
			if (topicDataStore !== undefined) {
				Logger.system.debug("getStore for topic", event);
	...
	this.save = function (event, cb) {
		var dataStore = self.getStore(event);

		if (event.data && self.storageObjects[dataStore]) {
			return self.storageObjects[dataStore].save(event.data, function (err, data) {
				return cb(err, data);
			});
		}
	};

	this.delete = function (event, cb) {
		var dataStore = self.getStore(event);

		if (event.data && self.storageObjects[dataStore]) {
			return self.storageObjects[dataStore].delete(event.data, function (err, data) {
				return cb(err, data);
			});
		}
	};

	this.clearCache = function (event, cb) {
		var dataStore = self.getStore(event);

		if (self.storageObjects[dataStore]) {
			console.log("call clear cache");
			return self.storageObjects[dataStore].clearCache(event.data, function (err, data) {
				return cb(err, data);
			});
		}
	};

	this.get = function (event, cb) {
		var dataStore = self.getStore(event);

		if (event.data && self.storageObjects[dataStore]) {
			return self.storageObjects[dataStore].get(event.data, function (err, data) {
				return cb(err, data);
			});
		}
	};

	this.keys = function (event, cb) {
		var dataStore = self.getStore(event);

		if (event.data && self.storageObjects[dataStore]) {
			return self.storageObjects[dataStore].keys(event.data, function (err, data) {
				return cb(err, data);
			});
		}
	};
	...
	var application = fin.desktop.Application.getCurrent();
	//This load the models from the finsemble config. The object is serialized so that it can go through the router.
	this.setupConfiguration = function (cb) {
		var self = this;
		ConfigClient.get({ field: "finsemble" }, function (err, finsemble) {
			self.setStore({ dataStore: finsemble.defaultStorage ? finsemble.defaultStorage : "localStorage" });
			var storageModels = finsemble.storage;
			if (!storageModels) {
				return;
			}
			var tasks = [];
			function addTask(path) {
				tasks.push(function (done) {
					Utils.injectJS(path, done); //inject the js into the page
				});
			}
			for (key in storageModels) {
				//add storage models provided
				addTask(storageModels[key]);
			}
			async.parallel(tasks, function (err) {
				if (err) {
					Logger.system.log(err);
				}
				self.loadModels(); // when all models are injected load them into the service
				cb();
			});
		});
	};
...
Storage.prototype = new baseService();
var serviceInstance = new Storage("storageService");
serviceInstance.onBaseServiceReady(function (callback) {
	Logger.system.log("onBaseServiceReady called");
	Logger.start();
	window.RouterClient = serviceInstance.RouterClient;
	serviceInstance.loadListeners();
	serviceInstance.setupConfiguration(callback);
	Logger.system.log("Storage Service Ready to go online");
	window.StorageService = serviceInstance;
});

serviceInstance.start();
window.storage = serviceInstance;

 ;(function register() { /* react-hot-loader/webpack */ if (process.env.NODE_ENV !== 'production') { if (typeof __REACT_HOT_LOADER__ === 'undefined') { return; } /* eslint-disable camelcase, no-undef */ var webpackExports = typeof __webpack_exports__ !== 'undefined' ? __webpack_exports__ : module.exports; /* eslint-enable camelcase, no-undef */ if (typeof webpackExports === 'function') { __REACT_HOT_LOADER__.register(webpackExports, 'module.exports', "E:\\chartiq\\finsemble\\src\\services\\storage\\storageService.js"); return; } /* eslint-disable no-restricted-syntax */ for (var key in webpackExports) { /* eslint-enable no-restricted-syntax */ if (!Object.prototype.hasOwnProperty.call(webpackExports, key)) { continue; } var namedExport = void 0; try { namedExport = webpackExports[key]; } catch (err) { continue; } __REACT_HOT_LOADER__.register(namedExport, key, "E:\\chartiq\\finsemble\\src\\services\\storage\\storageService.js"); } } })();
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
/* WEBPACK VAR INJECTION */(function(process) {var BaseStorage = __webpack_require__(29);
var Logger = __webpack_require__(1);
...
module.exports = LocalStorage; //Allows us to get access to the unintialized object
 ...
 ;(function register() { /* react-hot-loader/webpack */ if (process.env.NODE_ENV !== 'production') { if (typeof __REACT_HOT_LOADER__ === 'undefined') { return; } /* eslint-disable camelcase, no-undef */ var webpackExports = typeof __webpack_exports__ !== 'undefined' ? __webpack_exports__ : module.exports; /* eslint-enable camelcase, no-undef */ if (typeof webpackExports === 'function') { __REACT_HOT_LOADER__.register(webpackExports, 'module.exports', "E:\\chartiq\\finsemble\\src\\services\\storage\\models\\localStorage.js"); return; } /* eslint-disable no-restricted-syntax */ for (var key in webpackExports) { /* eslint-enable no-restricted-syntax */ if (!Object.prototype.hasOwnProperty.call(webpackExports, key)) { continue; } var namedExport = void 0; try { namedExport = webpackExports[key]; } catch (err) { continue; } __REACT_HOT_LOADER__.register(namedExport, key, "E:\\chartiq\\finsemble\\src\\services\\storage\\models\\localStorage.js"); } } })();
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
module.exports = Redis; //Allows us to get access to the unintialized object
 ...
 ;(function register() { /* react-hot-loader/webpack */ if (process.env.NODE_ENV !== 'production') { if (typeof __REACT_HOT_LOADER__ === 'undefined') { return; } /* eslint-disable camelcase, no-undef */ var webpackExports = typeof __webpack_exports__ !== 'undefined' ? __webpack_exports__ : module.exports; /* eslint-enable camelcase, no-undef */ if (typeof webpackExports === 'function') { __REACT_HOT_LOADER__.register(webpackExports, 'module.exports', "E:\\chartiq\\finsemble\\src\\services\\storage\\models\\redis.js"); return; } /* eslint-disable no-restricted-syntax */ for (var key in webpackExports) { /* eslint-enable no-restricted-syntax */ if (!Object.prototype.hasOwnProperty.call(webpackExports, key)) { continue; } var namedExport = void 0; try { namedExport = webpackExports[key]; } catch (err) { continue; } __REACT_HOT_LOADER__.register(namedExport, key, "E:\\chartiq\\finsemble\\src\\services\\storage\\models\\redis.js"); } } })();
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
