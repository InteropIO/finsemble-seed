...
var ConfigUtil = __webpack_require__(9);
var urlJoin = __webpack_require__(67);
var Logger = __webpack_require__(1);
var DataStoreClient = __webpack_require__(26);
DataStoreClient.initialize();

Logger.system.log("Starting Config Service");

function ConfigService() {
	var configStore;
	var manifest; // holds the complete processed manifest
	var self = this;

	// async read of JSON config file
	function getImportFile(importURL, importCallback) {
		Logger.system.debug("fetching import " + importURL);
		fetch(importURL, {
			credentials: "include"
		}).then(function (response) {
			return response.json();
  ...
  * @private
 */
	function processFinsembleConfig(params, callback) {
		let { manifest, allowOverwrites, recursionLevel, finsembleConfigObject, replaceComponents, replaceServices, updateStore } = params;
		ConfigUtil.resolveConfigVariables(manifest.finsemble, finsembleConfigObject); // resolve variable within finsemble config object
		Logger.system.debug("*** entering processFinsembleConfig ***", finsembleConfigObject, "recursionLevel", recursionLevel);
		...
		function filterNonSystemComponents(configObject) {
			...
			for (var key in configObject) {
				if (!configObject[key].component || !configObject[key].component.category || configObject[key].component.category !== "system") {
					Logger.system.debug("filterNonSystemComponents deleting", key);
					delete configObject[key];
					changed = true;
				}
			}
			Logger.system.debug("filterNonSystemComponents after", configObject, changed);
			return changed;
		}
		...
		function filterNonSystemServices(configObject) {
			...
			for (var key in configObject) {
				if (!configObject[key].category || configObject[key].category !== "system") {
					Logger.system.debug("filterNonSystemServices deleting", key);
					delete configObject.key;
					changed = true;
				}
			}
			Logger.system.debug("filterNonSystemServices after", configObject, changed);
			return changed;
		}
		...
		function conditionalStoreValue(updateStore, field, value) {
			if (updateStore) {
				configStore.setValue({ field, value });
			}
		}
   ...
   */
		function processImportConfig(configImportObject, allowOverwrites, processImportConfigCallback) {
			...
			Logger.system.debug("* processImportConfig *", configImportObject, "allowOverwrites", allowOverwrites);
			...
			function processURL(urlIndex, importCount) {
				Logger.system.debug("processURL", urlIndex, "importCount", importCount);
				if (urlIndex >= importCount) {
					// if no more import URLS to process in file, then invoke callback
					Logger.system.debug("---> importing all URLs complete", recursionLevel);
					...
					getImportFile(currentImportURL, function (error, newFinsembleConfigObject) {
						if (!error) {
							Logger.system.debug("---> importing into finsemble config", currentImportURL, "recursionLevel", recursionLevel);
							...
							processFinsembleConfig(PFCParams, function (manifest) {
								Logger.system.debug("updated manifest with configObject", currentImportURL, manifest);
							...
							console.error("failed importing into finsemble config", currentImportURL, error);
							processURL(++urlIndex, importCount);
						}
					});
				}
		...
		for (var key in finsembleConfigObject) {
			...
			if (commentRegex.test(key)) {
				...
				Logger.system.debug("ignoring comment", key);
				...
				if (allowOverwrites && replaceComponents) {
					...
					if (changed && updateStore) {
						configStore.setValue({ field: "finsemble.components", value: manifest.finsemble.components });
					}
				...
				for (var newComponent in finsembleConfigObject.components) {
					Logger.system.debug("adding component", newComponent);
					if (!allowOverwrites && manifest.finsemble.components.hasOwnProperty(newComponent)) {
						...
						Logger.system.info("finsemble component config", newComponent, "is being overwritten by component config", finsembleConfigObject.components[newComponent]);
				...
				if (allowOverwrites && replaceServices) {
					...
					if (changed && updateStore) {
						configStore.setValue({ field: "finsemble.services", value: manifest.finsemble.services });
					}
				...
				for (var newService in finsembleConfigObject.services) {
					Logger.system.debug("adding service", newService);
					if (!allowOverwrites && manifest.finsemble.services.hasOwnProperty(newService)) {
						...
						Logger.system.info("finsemble service config", newService, "is being overwritten by service config", finsembleConfigObject.components[newService]);
				...
				// doing nothing on first pass -- process importConfig URLs seperately at the end (see below)

			} else if (key === "importThirdPartyConfig") {
				// doing nothing on first pass -- process importThirdPartyConfig URLs seperately at the end (see below)

			} else {
				Logger.system.debug("adding key", key);
				if (!allowOverwrites && manifest.finsemble.hasOwnProperty(key)) {
					...
					Logger.system.info("finsemble config property", key, "is being overwritten");
		...
		if (recursionLevel > 10) {
			console.error("processFinsembleConfig is over 10 levels so may be in an import loop -- stopping at this level");
			...
			processImportConfig(finsembleConfigObject.importConfig, true, function () {
				processImportConfig(finsembleConfigObject.importThirdPartyConfig, false, function () {
					callback(manifest);
				});
			});
		}
  ...
  * @private
  */
	function processManifest(manifest, processManifestCallback) {
		if (typeof manifest.finsemble !== "undefined") {
			...
			console.error("finsemble property not found in manifest");
		}
	}
	/**
  *
  ...
  * @param {function} callback
  */
	function initializeDataStore(manifest, callback) {
		Logger.system.debug("initializeDataStore");
		DataStoreClient.createStore({
			...
			if (err) {
				...
				Logger.system.log("initializeDataStore complete", configStore);
				callback();
			}
		});
	}

	function finishConfigServiceInitialization() {
		Logger.system.debug("finishConfigServiceInitialization");

		// handles "get" request from ConfigClient
		RouterClient.addResponder("config.get", function (err, queryEvent) {
			var field;
			var errMsg = null;
			var responseData;
			var toGet;
			...
			function getValuesCallback(err, storeData) {
				if (err) {
					...
					Logger.system.warn(errMsg);
					queryEvent.sendQueryResponse(errMsg, null);
				} else {
					Logger.system.log("get responseData", storeData);
					queryEvent.sendQueryResponse(errMsg, storeData);
				}
			}

			// data to be retrieved stored in configReference
			console.log("config.get", queryEvent);
			if (queryEvent.data && queryEvent.data.hasOwnProperty("field")) {
				field = queryEvent.data.field;
				configStore.getValues({ field }, getValuesCallback); // this case returns a subfield in the store
			} else {
				configStore.getValues(getValuesCallback); // this case returns everything in the store
			}
		});

		RouterClient.addResponder("config.set", function (err, queryEvent) {
			console.log("config.set", queryEvent);
			...
			if (queryEvent.data && queryEvent.data.hasOwnProperty("field")) {
				if (!field.startsWith("finsemble.")) {
					queryEvent.sendQueryResponse("You cannot modify this part of the config", null);
				} else {
					configStore.setValues({ field: queryEvent.data.field, value: queryEvent.data.value });
					configStore.getValues({ field }, getValuesCallback);
				}
			} else {
		...
		RouterClient.addResponder("config.processAndSet", function (err, queryEvent) {
			var trustedRequest = RouterClient.trustedMessage(queryEvent);
			...
			Logger.system.debug("Config.processAndSet", trustedRequest, newConfig);
			...
			if (overwrite && !trustedRequest) {
				...
				processFinsembleConfig(PFCParams, function (manifest) {
					Logger.system.debug("Config.processAndSet revised manifest", manifest);
					// only by setting one last value and waiting for callback do we know all the previous setValues are done too
					configStore.setValue({ field: "finsemble.configSetDone", value: true }, function () {
						queryEvent.sendQueryResponse(null, manifest.finsemble);
					});
					RouterClient.publish("Config.changeNotification", { newConfig, finsemble: manifest.finsemble });
				});
			}
		});

		RouterClient.addPubSubResponder("Config.changeNotification", { newConfig: null, finsemble: manifest.finsemble });
	}
	...
	this.initialize = function (callback) {
		var self = this;
		ConfigUtil.getInitialManifest(function (initialManifest) {
			processManifest(initialManifest, function (completeManifest) {
				manifest = completeManifest;
				Logger.system.info("************* FULL PROCESSED MANIFEST *************", manifest);
				initializeDataStore(manifest, function () {
					Logger.system.debug("initializeDataStore complete");
					finishConfigServiceInitialization();
					callback();
				});
			});
		});
...
ConfigService.prototype = new baseService("configService");
var configServiceInstance = new ConfigService();

configServiceInstance.addNeededClients(["dataStoreClient"]);

configServiceInstance.onBaseServiceReady(function (callback) {
	Logger.system.log("onBaseServiceReady called");
	Logger.start();
	window.RouterClient = configServiceInstance.RouterClient;
	configServiceInstance.initialize(callback);
});

configServiceInstance.start();
window.ConfigService = configServiceInstance; // make accessible for convenience (private window)

 ;(function register() { /* react-hot-loader/webpack */ if (process.env.NODE_ENV !== 'production') { if (typeof __REACT_HOT_LOADER__ === 'undefined') { return; } /* eslint-disable camelcase, no-undef */ var webpackExports = typeof __webpack_exports__ !== 'undefined' ? __webpack_exports__ : module.exports; /* eslint-enable camelcase, no-undef */ if (typeof webpackExports === 'function') { __REACT_HOT_LOADER__.register(webpackExports, 'module.exports', "E:\\chartiq\\finsemble\\src\\services\\config\\configService.js"); return; } /* eslint-disable no-restricted-syntax */ for (var key in webpackExports) { /* eslint-enable no-restricted-syntax */ if (!Object.prototype.hasOwnProperty.call(webpackExports, key)) { continue; } var namedExport = void 0; try { namedExport = webpackExports[key]; } catch (err) { continue; } __REACT_HOT_LOADER__.register(namedExport, key, "E:\\chartiq\\finsemble\\src\\services\\config\\configService.js"); } } })();
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
