...
Logger.system.log("Starting Authentication Service");
...
function AuthenticationService() {
	...
	function startAuthentication(startCallBack) {
		function waitForAuthorization() {
			Logger.system.log("Waiting for Authorization");
			RouterClient.addListener("AuthenticationService.authorization", function (error, response) {
				...
				if (error) {
					Logger.system.error("Error on incoming authorization: " + JSON.stringify(err));
				} else {
					Logger.system.log("Authorization: " + JSON.stringify(response.data));
					if (firstAuthorization) {
						firstAuthorization = false;
						startCallBack();
					}
					var authorization = response.data;
					StorageClient.setUser({ user: authorization.user });
					RouterClient.publish("Authorization", authorization);
				}
			});
		}

		Logger.system.debug("starting Authentication");
		RouterClient.addPubSubResponder("Authorization", { "state": "waiting" });

		RouterClient.subscribe("Authorization", function (err, response) {
			if (err) {
				Logger.system.error("Failed Authorization Notification: " + JSON.stringify(err));
			} else {
				// spawn successful so wait for response
				Logger.system.info("Authorization Notification: " + JSON.stringify(response.data));
			}
		});

		ConfigClient.get({ field: "finsemble.isAuthEnabled" }, function (err, isAuthEnabled) {
			Logger.system.log("finsemble.isAuthEnabled", isAuthEnabled);
			if (isAuthEnabled) {
				Logger.system.info("Authentication Enabled");
				...
				LauncherClient.spawn("defaultAuthentication", {}, function (err, msg) {
					Logger.system.info("Authentication spawned");
					if (err) {
						Logger.system.error("Failed To Spawn Authentication Component: " + JSON.stringify(err));
					}
				});
				waitForAuthorization();
			} else {
				Logger.system.info("Authentication Disabled");
				var defaultUser = "defaultUser";
				StorageClient.setUser({ user: defaultUser });
	...
	function startServingSignOn(startSignOnCallback) {
		Logger.system.debug("startServingSignOn");
		var activeSignOns = {};
		var nowSignedOn = {};
		var acceptTimers = {};
		...
		function setUpResponseTimeout(signOnKey) {
			Logger.system.debug("acceptTimeout started", signOnKey);
			...
			function acceptTimeout() {
				Logger.system.debug("acceptTimeout fired", timerSignOnKey);
		...
		function clearResponseTimeout(signOnKey) {
			Logger.system.debug("acceptTimeout cleared", signOnKey);
		...
		function continueSignOn(queryEvent, signOnKey, params, signOnData, retry) {
			Logger.system.debug("continueSignOn", queryEvent, signOnKey, params, signOnData, retry);
			...
			if (activeSignOnObject === undefined || retry) {
				...
				if (signOnData) {
					...
					Logger.system.debug("continueSignOn using stored signOnData", signOnData);
					if (isSignedOn) {
					...
					Logger.system.debug("authentication.appSignOn spawning dialog");
					LauncherClient.spawn("dialogSignOn", { data: { signOnKey, params } }, function (err, msg) {
						if (err) {
							Logger.system.error("Failed To Spawn Authentication Component: " + JSON.stringify(err));
							...
							// spawn successful so wait for response
							Logger.system.debug("Authentication SignOn Successfully Spawned");
						}
					});
					activeSignOns[signOnKey] = activeSignOnObject;
				}
				...
				Logger.system.debug("continueSignOn queuing request", queryEvent);
		...
		RouterClient.addResponder("authentication.appSignOn", function (err, queryEvent) {
			...
			Logger.system.debug("authentication.appSignOn", signOnKey, params, queryEvent);
			...
			if (!params.force) {
				if (signOnKey in autoSignOnConfig.hardcoded) {
					...
					Logger.system.debug("authentication.appSignOn hardcoded " + JSON.stringify(hardcodedSignon));
					continueSignOn(queryEvent, signOnKey, params, hardcodedSignon, false);
				} else {
					StorageClient.get({ topic: "finsemble", key: getSignOnStorageKey(signOnKey) }, function (err, encryptedData) {
						if (encryptedData) {
				...
				Logger.system.debug("authentication.appSignOn forcing signOn");
				continueSignOn(queryEvent, signOnKey, params, null, false);
			}
		});
		...
		RouterClient.addResponder("authentication.appRejectAndRetrySignOn", function (err, queryEvent) {
			Logger.system.debug("authentication.appRejectAndRetrySignOn", err, queryEvent);
			var signOnKey = queryEvent.data.signOnKey;
			...
			if (activeSignOnObject !== undefined) {
				...
				Logger.system.warn("authentication.appRejectAndRetrySignOn for inactive signOnKey:", signOnKey);
			}
		});
		...
		RouterClient.addListener("authentication.dialogSignOnToAuthService", function (err, signOnEvent) {
			Logger.system.debug("authentication.dialogSignOnToAuthService", err, signOnEvent);
			...
			if (!err) {
				...
				signOnData.validationRequired = true;
				Logger.system.debug("authentication.dialogSignOnToAuthService response" + JSON.stringify(signOnData));
		...
		RouterClient.addListener("authentication.appAcceptSignOn", function (err, transmitEvent) {
			Logger.system.debug("authentication.appAcceptSignOn", err, transmitEvent);
			...
			if (activeSignOnObject !== undefined) {
				...
				Logger.system.debug("appAcceptSignOn Saving in Storage", activeSignOnObject.signOnData);

				myCrypto.encryptString(JSON.stringify(activeSignOnObject.signOnData), function (encryptedData) {
					Logger.system.debug("appAcceptSignOn After Encryption", activeSignOnObject.signOnData);
					StorageClient.save({ topic: "finsemble", key: getSignOnStorageKey(signOnKey), value: encryptedData });

					for (var i = 0; i < activeSignOnObject.queuedQuerys.length; i++) {
						Logger.system.debug("authentication.appAcceptSignOn", i, activeSignOnObject.signOnData);
				...
				console.warn("authentication.appAcceptSignOn for inactive signOnKey", signOnKey);
			}
		});

		// handles client appRejectSignOn request
		RouterClient.addListener("authentication.appRejectSignOn", function (err, transmitEvent) {
			Logger.system.debug("authentication.appRejectSignOn", err, transmitEvent);
			var signOnKey = transmitEvent.data.signOnKey;
			var activeSignOnObject = activeSignOns[signOnKey];
			if (activeSignOnObject !== undefined) {
				clearResponseTimeout(signOnKey);
				...
				console.warn("authentication.appRejectSignOn for inactive signOnKey", signOnKey);
			}
		});
		ConfigClient.get({ field: "finsemble.autoSignOn" }, function (err, getResult) {
			if (err) {
				autoSignOnConfig = { hardcoded: {} };
			} else {
				autoSignOnConfig = getResult;
			}
			Logger.system.log("hardcoded", autoSignOnConfig);
	...
	// this a message from launcher telling authentication that it's ready.
	this.RouterClient.subscribe("Finsemble.AuthenticationService.activate", function (err, response) {
		if (response.data.activate) {
			Logger.system.debug("starting Authentication");
			self.initialize(function () {
				self.RouterClient.publish("Finsemble.AuthenticationService.activated", { activated: true });
			});
		}
	});
...
function MyCrypto(inputSimpleKey, ready) {
	...
	["encrypt", "decrypt"] //can be "encrypt", "decrypt", "wrapKey", or "unwrapKey"
		...
		Logger.system.debug("importKey success", mySimpleKey, getSeedKey(mySimpleKey), "K", key, "M", myKey);
		ready();
	}).catch(function (err) {
		Logger.system.error("importKey failure", err);
		//band-aid. Crypto doesn't work on insecure sites.
		ready();
	});

	this.encryptString = function (stringToEncrypt, callback) {
		Logger.system.debug("encryptString", "stringToEncrypt", stringToEncrypt); // <<<<---------- REMOVE AFTER DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		window.crypto.subtle.encrypt({
			...
			Logger.system.debug("encryptString", "encryptedData", encryptedData, encryptedString);
	...
	this.decryptString = function (encryptedString, callback) {
		...
		Logger.system.debug("decryptString", "encryptedData", encryptedData, encryptedString);
		window.crypto.subtle.decrypt({
			name: "AES-CBC",
			iv: myIv
		}, myKey, encryptedData).then(function (decrypted) {
			Logger.system.debug("decryptString", "decrypted", decrypted); // <<<<---------- REMOVE AFTER DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			var decryptedString = new TextDecoder("utf-8").decode(new Uint8Array(decrypted));
			Logger.system.debug("decryptString", "decryptedString", decryptedString); // <<<<---------- REMOVE AFTER DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			callback(decryptedString);
		}).catch(function (err) {
			Logger.system.error("decryptString error", err, "myKey", myKey, "encryptedData", encryptedData, "encryptedString", encryptedString);
		});
	};
}
	...
	var crypto = new MyCrypto("mySimpleTestKey1", function () {
		crypto.encryptString(stringToEncrypt, function (encryptedString) {
			crypto.decryptString(encryptedString, function (myString) {
				Logger.system.debug("*** RESULT 1------>", myString);
			});
		});
	});
}
...
function cyptoTest2() {
	...
	var crypto = new MyCrypto("09587BJDlfakjMGWROY", function () {
		crypto.encryptString(stringToEncrypt, function (encryptedString) {
			StorageClient.save({ topic: "finsemble", key: "cyptostoraget2", value: encryptedString }, function () {
				StorageClient.get({ topic: "finsemble", key: "cyptostoraget2" }, function (err, storedEncryptedString) {
					var parsed = JSON.parse(storedEncryptedString);
					crypto.decryptString(parsed, function (myString) {
						Logger.system.debug("*** RESULT 2------>", myString);
					});
				});
			});
		});
...
function cyptoTest3() {
	...
	var crypto1 = new MyCrypto("lkwgrjOIHJfnqoehfdf", function () {
		crypto1.encryptString(stringToEncrypt, function (encryptedString) {
			StorageClient.save({ topic: "finsemble", key: "cyptostoraget3", value: encryptedString }, function () {
				var crypto2 = new MyCrypto("lkwgrjOIHJfnqoehfdf", function () {
					StorageClient.get({ topic: "finsemble", key: "cyptostoraget3" }, function (err, storedEncryptedString) {
						var parsed = JSON.parse(storedEncryptedString);
						crypto2.decryptString(parsed, function (myString) {
							Logger.system.debug("*** RESULT 3------>", myString);
						});
					});
				});
			});
...
AuthenticationService.prototype = new baseService("authenticationService");
var serviceInstance = new AuthenticationService();

serviceInstance.addNeededServices(["launcherService", "storageService"]);
...
module.exports = serviceInstance;
 ...
 ;(function register() { /* react-hot-loader/webpack */ if (process.env.NODE_ENV !== 'production') { if (typeof __REACT_HOT_LOADER__ === 'undefined') { return; } /* eslint-disable camelcase, no-undef */ var webpackExports = typeof __webpack_exports__ !== 'undefined' ? __webpack_exports__ : module.exports; /* eslint-enable camelcase, no-undef */ if (typeof webpackExports === 'function') { __REACT_HOT_LOADER__.register(webpackExports, 'module.exports', "E:\\chartiq\\finsemble\\src\\services\\authentication\\authenticationService.js"); return; } /* eslint-disable no-restricted-syntax */ for (var key in webpackExports) { /* eslint-enable no-restricted-syntax */ if (!Object.prototype.hasOwnProperty.call(webpackExports, key)) { continue; } var namedExport = void 0; try { namedExport = webpackExports[key]; } catch (err) { continue; } __REACT_HOT_LOADER__.register(namedExport, key, "E:\\chartiq\\finsemble\\src\\services\\authentication\\authenticationService.js"); } } })();
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
