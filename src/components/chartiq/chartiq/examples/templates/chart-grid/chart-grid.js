...
function SetChartSize(element, width, height, xaxis) {
	...
	element.style.height =
		"calc(" + height + "vh - " + ((height / 100) * 50 + margin) + "px)";
}
...
function handleChartReady(evt) {
	...
	// We'll do this as long as we don't support ES2018 because of MS Edge Legacy,
	// which doesn't support object spread literal in destructure
	...
	CIQ.extend(config, { callbacks, initialSymbol, restore, enabledAddOns });
	// Create chart
		...
		if (targetSymbol) {
			...
			if (symbolData.name === activeMarket) {
				...
				if (chartLinker.charts.size === 1) {
					...
					CIQ.alert("To set a chart symbol, first click on a chart to select.");
				}
			} else {
				CIQ.alert(
					"The symbol " +
...
CIQ.ChartEngine.prototype.prepend("mouseWheel", function (e) {
	let targetInstantChart = $(e.currentTarget.stx.container)
