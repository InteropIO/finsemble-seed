/* removeIf(umd) */ (function(root, factory) {
	...
	class Orderbook extends CIQ.UI.ModalTag {
		...
		disconnectedCallback(){
			if(this.context){
				...
				CIQ.UI.unobserveProperty("touched", stx.chart.currentMarketData, this.listener);
			}
		...
		setContext(context) {
			...
			CIQ.UI.observeProperty("touched", stx.chart.currentMarketData, this.listener);
		}
		...
		update(params){
			...
			var bidData=this.createMatrix(bids.Price_Size.slice().sort(sortFcn).reverse());
	...
	CIQ.UI.Orderbook = Orderbook;
	customElements.define("cq-orderbook", Orderbook);
