...
xignite.isFuturesSymbol = function (symbol) {
	if (!symbol) return false;
	if (symbol.indexOf("/") == 0) return true;
	return false;
};

xignite.isForexSymbol = function (symbol) {
	if (!symbol) return false;
...
xignite.isForexMetal = function (symbol) {
	if (!symbol) return false;
	if (!xignite.isForexSymbol(symbol)) return false;
	var tSym = symbol;
...
xignite.securityType = function (symbol, useSuperQuotes) {
	if (xignite.isIndexSymbol(symbol)) return "INDEX_REALTIME";
	if (xignite.isForexMetal(symbol)) return "METAL";
	if (xignite.isForexSymbol(symbol)) return "CURRENCY";
	if (xignite.isFuturesSymbol(symbol)) return "FUTURE";
	if (xignite.hasExchange(symbol)) return "EXCHANGE";
 ...
 */
xignite.getQuotes = function (symbols, entitlements, cb, identifierType, useSuperQuotes, extraParams) {
	if (!apiToken && needsEncryption) {
		if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
			setTimeout(function (self, args) {
				return function () {
					xignite.getQuotes.apply(self, args);
				};
			}(this, arguments), 100);
			return;
		}
	...
	var urls = {
		METAL: xignite.makeUrl("/globalmetals_xignite/xGlobalMetals.json/GetRealTimeMetalQuotes?Symbols={symbols}&Currency=&_fields=Symbol,Currency,Date,Time,Ask,Bid,Mid,Unit"),
		CURRENCY: xignite.makeUrl("/globalcurrencies_xignite/xGlobalCurrencies.json/GetRealTimeRates?Symbols={symbols}&_fields=Symbol,Date,Time,Ask,Bid,Mid,QuoteCurrency,BaseCurrency,Spread"),
		INDEX_REALTIME: xignite.makeUrl("/globalindicesrealtime_xignite/xglobalindicesrealtime.json/GetRealTimeIndicesValue?IdentifierType={identifierType}&Identifiers={symbols}"),
		FUTURE: xignite.makeUrl("/www_xignite/xFutures.json/GetDelayedFutures?Symbols={symbols}&Month=0&Year=0&_fields=Future.Symbol,Date,Time,Open,High,Low,Last,Volume,PreviousClose,Change,PercentChange"), // todo, determine forward month
		BATS: xignite.makeUrl("/batsrealtime_xignite/v3/xBATSRealTime.json/GetExtendedQuotes?IdentifierType={identifierType}&Identifiers={symbols}&_fields=Security.Symbol,Security.Name,Date,Time,Open,High,Low,Last,Volume,PreviousClose,ChangeFromPreviousClose,PercentChangeFromPreviousClose"),
		//BATS: xignite.makeUrl("/batsrealtime_xignite/xBATSRealTime.json/GetRealQuotes?Symbols={symbols}&_fields=Security.Symbol,Date,Time,Open,High,Low,Last,Volume,PreviousClose,Change,PercentChange"),
		CONSOLIDATED: xignite.makeUrl("/globalrealtime_xignite/v3/xGlobalRealTime.json/GetGlobalExtendedQuotes?IdentifierType={identifierType}&Identifiers={symbols}&_fields=Security.Symbol,Security.Name,Date,Time,Open,High,Low,Last,Volume,PreviousClose,ChangeFromPreviousClose,PercentChangeFromPreviousClose"),
		//CONSOLIDATED: xignite.makeUrl("/globalrealtime_xignite/v3/xGlobalRealTime.json/GetGlobalRealTimeQuotes?Symbols={symbols}&_fields=Security.Symbol,Date,Time,Open,High,Low,Last,Volume,PreviousClose,Change,PercentChange"),
		DELAYED: xignite.makeUrl("/globalquotes_xignite/v3/xGlobalQuotes.json/GetGlobalExtendedQuotes?IdentifierType={identifierType}&Identifiers={symbols}&_fields=Security.Symbol,Security.CUSIP,Security.ISIN,Security.CIK,Security.Valoren,Security.Name,Date,Time,Open,High,Low,Last,Volume,PreviousClose,Currency,UTCOffset,High52Weeks,Low52Weeks,Outcome,ChangeFromPreviousClose,PercentChangeFromPreviousClose"),
		...
		SUPERQUOTES: xignite.makeUrl("/superquotes_xignite/xSuperQuotes.json/GetQuotes?IdentifierType={identifierType}&Identifiers={symbols}"),
		METAL_HISTORICAL: xignite.makeUrl("/globalmetals_xignite/xGlobalMetals.json/GetHistoricalMetalQuotes?Symbols={symbols}&Currency=&PriceType=Mid&AsOfDate=" + asOfDate + "&FixingTime=" + fixingTime),
		CURRENCY_HISTORICAL: xignite.makeUrl("/globalcurrencies_xignite/xGlobalCurrencies.json/GetHistoricalRates?Symbols={symbols}&PriceType=Mid&AsOfDate=" + asOfDate + "&FixingTime=" + fixingTime),
	};
	...
	var indexUrls = {
		INDEX_DELAYED: xignite.makeUrl("/globalindices_xignite/xglobalindices.json/GetDelayedIndicesValue?IdentifierType={identifierType}&Identifiers={symbols}"),
		INDEX_HISTORICAL: xignite.makeUrl("/globalindiceshistorical_xignite/xglobalindiceshistorical.json/GetLastClosingIndicesValue?IdentifierType={identifierType}&Identifiers={symbols}"),
	};
	...
	for (var i = 0; i < symbols.length; i++) {
		...
		var securityType = xignite.securityType(symbol, useSuperQuotes);
	...
	function processResponse(status, result, remainingUrls, previousResults, cb) {
				...
				// Forex, Metals, BATS
					...
					if (item.BaseCurrency || item.Unit) {
						if (item.Time) { // RealTime
							...
							if (existingItem) {
								_.assign(fieldsToChange, existingItem)
							}
						}
					...
					else if (item.Outcome != 'RequestError') {
						arr[item.Symbol] = item;
					}
				}
				// Superquotes
		...
		if (_.size(remainingUrls)) {
			...
			for(var i in arr){
				if(arr[i].Last){
					window.cachedQuotes[i] = _.assign(window.cachedQuotes[i], arr[i]);
				}
			}
...
xignite.fetchQuotes = function (symbols, cb, extraParams) {
	xignite.getQuotes(symbols, null, cb, null, true, extraParams);
}
...
xignite.fetchTopMovers = function (settings, moverType, moversCB, markets, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchTopMovers.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	...
	var marketToExchangeCode = {
		'US': 'XNYS,XNAS,XASE,ARCX',
		'UK': 'XLON',
		'IN': 'XNSE',
		'JP': 'XTKS',
		'DE': 'XFRA'
	...
	if (!markets) markets = ['US'];

	var marketsText = '';
	_.each(markets, function (value) {
		if (marketToExchangeCode[value]) marketsText += marketToExchangeCode[value];
		else marketsText += value;
	});

	var url = xignite.makeUrl("/globalquotes_xignite/v3/xGlobalQuotes.json/GetTopMarketMovers?MarketMoverType=" + moverTypeCodeMap[moverType] + "&NumberOfMarketMovers=50&Exchanges=" + marketsText + "&_fields=Movers.Symbol,Movers.Last,Movers.ChangeFromPreviousClose,Movers.MarketIdentificationCode");
	PortalCore.serverFetch(url, null, null, function (status, result) {
		...
		if (result.Movers) {
			for (var s = 0; s < result.Movers.length; s++) {
				...
				if (item.Last < 1) continue;
				item.Previous = item.Last - item.ChangeFromPreviousClose;
				if (!_.includes(['XNYS', 'XNAS', 'XASE', 'ARCX'], item.MarketIdentificationCode)) {
					item.Symbol = item.Symbol + '.' + item.MarketIdentificationCode;
				}
...
xignite.fetchRates = function (settings, cb, extraParams) {
	var ratesObject = {};

	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchRates.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	...
	_.each(familyNames, function (family, ciqfamily) {
		(function (xifamily) {
			var items = {};
			var url = xignite.makeUrl("/www_xignite/xRates.json/GetLatestRateFamily?RateFamilyType=" + xifamily + "&_fields=Type,Text");
			PortalCore.serverFetch(url, null, null, function (status, result) {
				if (status != 200) return;
				result = JSON.parse(result);
				var count = 0;
				...
				//cb();
				var url2 = xignite.makeUrl("/www_xignite/xRates.json/GetHistoricalRateFamily?RateFamilyType=" + xifamily + "&FromDate=" + PortalCore.mmddyyyy(start) + "&ToDate=" + PortalCore.mmddyyyy(end) + "&_fields=HistoricalInterestRate.Type,HistoricalInterestRate.Rates.Date,HistoricalInterestRate.Rates.Text");
...
xignite.fetchHistoricalRates = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchHistoricalRates.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	...
	var url2 = xignite.makeUrl("/www_xignite/xRates.json/GetHistoricalRateFamily?RateFamilyType=" + familyNames[settings.familyName] + "&FromDate=" + PortalCore.mmddyyyy(start) + "&ToDate=" + PortalCore.mmddyyyy(end));
	PortalCore.serverFetch(url2, null, null, function (status, result) {
		if (status != 200) return;
		result = JSON.parse(result);
		var items = {};
...
xignite.sectorSymbolTransform = function (item) {
...
xignite.fetchSectorPerformance = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchSectorPerformance.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	...
	var url = xignite.makeUrl("/superquotes_xignite/xSuperQuotes.json/GetQuotes?IdentifierType=Symbol&Identifiers=" + sectorTickers + "&_fields=Identifier,Last,PreviousClose,DateTime,PreviousCloseDate");
	PortalCore.serverFetch(url, null, null, function (status, result) {
		...
		for (var s = 0; s < result.length; s++) {
			...
			if (item.Identifier) item.Symbol = xignite.sectorSymbolTransform(item.Identifier);
		...
		//cb(null, items, extraParams);
		//cb(false);
		var url2 = xignite.makeUrl("/globalquotes_xignite/v3/xGlobalQuotes.json/GetTopMarketMovers?MarketMoverType=MostActive&NumberOfMarketMovers=50&Exchanges=XNYS,XNAS,XASE,ARCX&_fields=Movers.Symbol,Movers.Last,Movers.ChangeFromPreviousClose");
		PortalCore.serverFetch(url2, null, null, function (status, result) {
			if (status != 200) return;
			result = JSON.parse(result);
			var items = {};
			...
			var url3 = xignite.makeUrl("/factsetfundamentals_xignite/xFactSetFundamentals.json/GetLatestFundamentals?IdentifierType=Symbol&Identifiers=" + symbols.join(",") + "&FundamentalTypes=Industry&UpdatedSince=&_fields=Company.Symbol,Company.Sector,Company.Industry");
			(function (items) {
		...
		PortalCore.serverFetch(url4, null, null, function (status, result) {
			...
			for (var s = 0; s < result.length; s++) {
				if (result[s].Company) {
					var symbol = xignite.sectorSymbolTransform(result[s].Company.Symbol);
...
xignite.requestToXigniteFundamentalsMap = {
	Previous: "LastClose",
...
xignite.requestToXigniteFundFundamentalsMap = {
...
xignite.processXigniteFundamentals = function (stock, mappedFundamentals, unmappedFundamentals, dfd) {
	require(['modules/xignite-fundamentallist'], function () {
		if (stock) {
...
xignite.buildFundamentalList = function (stock, data, dfd) {
	var fundamentals;
	if (stock) fundamentals = this.requestToXigniteFundamentalsMap;
	else fundamentals = this.requestToXigniteFundFundamentalsMap;
	...
	if (unmappedFundamentals.length) {
		this.processXigniteFundamentals(stock, mappedFundamentals, unmappedFundamentals, dfd);
	} else {
		dfd.resolve(_.join(mappedFundamentals));
	}
...
xignite.isMutual = function (symbol) {
...
xignite.fetchStockFundamentals = function (symbol, fundamentalList, item, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchStockFundamentals.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
				...
				for (var f = 0; f < fs.length; f++) {
					...
					for (var d = 0; d < fund.length; d++) {
						...
						if (fund[d].Type == "PercentPriceChange52Weeks") {
							...
							var key = _.findKey(xignite.requestToXigniteFundamentalsMap, _.partial(_.isEqual, fund[d].Type));
...
xignite.fetchFundFundamentals = function (symbol, instrumentType, fundamentalList, item, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchFundFundamentals.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	...
	if (instrumentType == "Fund" || instrumentType == "ExchangeTradedFund") {
		item.isMutual = xignite.isMutual(symbol);
		...
		$.when(dfd).done(function (joinedfundamentalList) {
			...
			var url3 = xignite.makeUrl("/fundfundamentals_xignite/xFundFundamentals.json/GetFundFundamentalList?IdentifierType=Symbol&Identifier=" + encodeURIComponent(symbol) + "&FundamentalTypes=" + joinedfundamentalList + "&UpdatedSince=&_fields=Fundamentals.Type,Fundamentals.Value");
			...
			PortalCore.serverFetch(url3, null, null, function (status, result) {
				...
				if (fund) {
					for (var d = 0; d < fund.length; d++) {
						...
						if (fund[d].Type == "FundManagersProfile") {
							...
							if (!item.AssetAllocation) item.AssetAllocation = { 'Net': {}, 'Long': {}, 'Short': {} };
							var w = fund[d].Type.split("Net");
							if (w.length == 2) {
								w[0] = w[0].replace(/(.)([A-Z])([a-z])/g, "$1 $2$3").replace(/(.)([a-z])([A-Z])(.)/g, "$1$2 $3$4");
								item.AssetAllocation.Net[w[0]] = fund[d].Value;
							}
							w = fund[d].Type.split("Long");
							if (w.length == 2) {
								w[0] = w[0].replace(/(.)([A-Z])([a-z])/g, "$1 $2$3").replace(/(.)([a-z])([A-Z])(.)/g, "$1$2 $3$4");
								item.AssetAllocation.Long[w[0]] = fund[d].Value;
							}
							w = fund[d].Type.split("Short");
							if (w.length == 2) {
								w[0] = w[0].replace(/(.)([A-Z])([a-z])/g, "$1 $2$3").replace(/(.)([a-z])([A-Z])(.)/g, "$1$2 $3$4");
								item.AssetAllocation.Short[w[0]] = fund[d].Value;
							}
							...
							var key = _.findKey(this.requestToXigniteFundFundamentalsMap, _.partial(_.isEqual, fund[d].Type));
							if (typeof key !== 'undefined') {
								item[key] = fund[d].Value;
							} else {
								item[fund[d].Type] = fund[d].Value;
				...
				else xignite.fetchStockFundamentals(symbol, fundamentalList, item, cb, extraParams);
			});
		});
	}
...
xignite.fetchDetailedQuote = function (symbol, fundamentalList, quoteCB, fundamentalsCB, extraParams) {
	if (symbol == "") return;
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchDetailedQuote.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	...
	var url = xignite.makeUrl("/superquotes_xignite/xSuperQuotes.json/GetQuote?IdentifierType=Symbol&Identifier=" + encodeURIComponent(symbol) + "&_fields=Name,Identifier,DateTime,UTCOffset,Last,Volume,PreviousClose,Change,PercentChange,High,Low,InstrumentType");
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status != 200) return;
		result = JSON.parse(result);
		var item = {};
		...
		if (result.InstrumentType == "Fund" && xignite.isMutual(symbol)) {
			var endDate = new Date();
			...
			var url2 = xignite.makeUrl("/navs_xignite/v2/xnavs.json/GetHistoricalNAVs?IdentifierType=Symbol&Identifier="+encodeURIComponent(symbol)+"&StartDate="+encodeURIComponent(startDate)+"&EndDate="+encodeURIComponent(endDate)+"&AdjustmentMethod=None&_fields=NAVs.NAV");
			PortalCore.serverFetch(url2, null, null, function (status, result2) {
				if (status != 200) return;
				result2 = JSON.parse(result2);
				if (result2 && result2.NAVs && result2.NAVs.length) {
					result2.NAVs.reverse();  // results are oldest to newest
					for (var l = 0; l < result2.NAVs.length; l++) {
						var last = result2.NAVs[l].NAV;
				...
				if (fundamentalList && fundamentalList.fund) {
					xignite.fetchFundFundamentals(symbol, result.InstrumentType, fundamentalList, item, fundamentalsCB, extraParams);
			...
			quoteCB(null, item, extraParams);
			if (fundamentalList) {
				if (fundamentalList.fund && (result.InstrumentType == "Fund" || result.InstrumentType == "ExchangeTradedFund")) xignite.fetchFundFundamentals(symbol, result.InstrumentType, fundamentalList, item, fundamentalsCB, extraParams);
				else if (fundamentalList.stock && result.InstrumentType != "Fund" && result.InstrumentType != "ExchangeTradedFund") xignite.fetchStockFundamentals(symbol, fundamentalList, item, fundamentalsCB, extraParams);
			}
		}
...
xignite.fetchFinancialStatements = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchFinancialStatements.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	...
	_.each(terms, function (term) {
		...
		xignite.fetchFinancialStatementsByTerm(termSettings, cb, extraParams);
	});
}

xignite.fetchFinancialStatementsByTerm = function (settings, cb, extraParams) {
	if (needsEncryption && !xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchFinancialStatementsByTerm.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	...
	var url = xignite.makeUrl("/factsetfundamentals_xignite/xFactSetFundamentals.json/GetFundamentalsFiscalRange?IdentifierType=Symbol&Identifiers=" + encodeURIComponent(settings.symbol) + "&FundamentalTypes=" + fundamentalTypes + "&StartDate=" + PortalCore.mmddyyyy(startDate) + "&EndDate=" + PortalCore.mmddyyyy(endDate) + "&ExcludeRestated=false&ReportType=" + term + "&UpdatedSince=");
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status != 200) return;
		result = JSON.parse(result);
		result = result[0];
...
xignite.fetchFinancialStatementsOld = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchFinancialStatementsOld.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
...
xignite.fetchFinancialStatement = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchFinancialStatement.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	...
	function fetchStatement(asOfDate) {
		var url = xignite.makeUrl("/factsetfundamentals_xignite/xFactSetFundamentals.json/Get" + statement + "s?IdentifierType=Symbol&Identifiers=" + encodeURIComponent(symbol) + "&ReportType=" + term + "&AsOfDate=" + PortalCore.mmddyyyy(asOfDate) + "&ExcludeRestated=false&UpdatedSince=");
		PortalCore.serverFetch(url, null, null, function (status, result) {
			if (status != 200) return;
			result = JSON.parse(result);
...
xignite.fetchFundamentalsFiscalRange = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchFundamentalsFiscalRange.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
...
xignite.fetchSecurityInsiders = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchSecurityInsiders.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
...
xignite.fetchSecurityFilings = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchSecurityFilings.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	...
	var url = xignite.makeUrl("/www_xignite/xEdgar.json/SearchFilings?Identifier=" + encodeURIComponent(settings.symbol) + "&IdentifierType=Symbol&PriorToDate=" + PortalCore.mmddyyyy(end) + "&OwnershipForms=Include&Form=&OutputType=AllMatches&_fields=Filings.Date,Filings.Type,Filings.Description,Filings.HtmlFileUrl");
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status != 200) return;
		result = JSON.parse(result);
		if (!result.Filings) {
...
xignite.getETUTCOffset = function (date) {
...
xignite.fetchEarningsCalendar = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchEarningsCalendar.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}

	if (!settings.date) settings.date = new Date();

	var url = xignite.makeUrl("/www_xignite/xEarningsCalendar.json/GetEarningCalendarDay?MinimumMarketCap=1000000000&AsOfDate=" + PortalCore.mmddyyyy(settings.date));
	...
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status == 200) {
			...
			if (result.Announcements && result.Announcements.length) {
				...
				_.each(result.Announcements, function (value, key) {
					var item = {}
					item.Symbol = value.Security.Symbol;
					item.Name = value.Security.Name;
					item.EarningsDate = value.EarningsDate;
					item.EarningsQuarter = value.EarningsQuarter;
					item.TimeType = value.TimeType;
					if (_.includes(value.ConferenceCall.Time, 'T')) {
						var dateTimeSplit = value.ConferenceCall.Time.split('T');
						item.ConferenceCallDate = dateTimeSplit[0];
						item.ConferenceCallTime = dateTimeSplit[1];
					} else {
						item.ConferenceCallDate = value.ConferenceCall.Date;
						item.ConferenceCallTime = value.ConferenceCall.Time;
					}

					if (item.ConferenceCallDate && item.ConferenceCallTime) {
						var dateParts = item.ConferenceCallDate.split("/");
						var timeParts = item.ConferenceCallTime.split(/[ :]/);
						if (timeParts[2] == 'PM' && timeParts[0] != '12') timeParts[0] = parseInt(timeParts[0]) + 12;
						item.ConferenceCallDateTime = new Date(dateParts[2], dateParts[0] - 1, dateParts[1], timeParts[0], timeParts[1], 0, 0);

					} else {
						item.ConferenceCallDateTime = ''
					}

					item.ConferenceCallLink = value.ConferenceCall.BroadcastURL;
					items.push(item);
				});
...
xignite.fetchEconomicCalendar = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchEconomicCalendar.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	...
	PortalCore.serverFetch(url, null, null, function (status, result) {
		...
		for (hol in holidayExchanges) {
			var url2 = makeUrl("/globalholidays_xignite/xGlobalHolidays.json/GetHolidays?Location=" + holidayExchanges[hol] + "&LocationType=Exchange&StartDate=" + PortalCore.mmddyyyy(start) + "%20" + PortalCore.hhmmss(start) + "&EndDate=" + PortalCore.mmddyyyy(end) + "%20" + PortalCore.hhmmss(end) + "&_fields=Date,Name,FullHoliday,EarlyClose");
			(function (country) {
				PortalCore.serverFetch(url2, null, null, function (status, result) {
					if (status == 200) {
...
xignite.fetchEconomicCalendarDetail = function (settings, cb, extraParams) {
	if (needsEncryption && !setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchEconomicCalendarDetail.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}
	var url = xignite.makeUrl("/www_xignite/xCalendar.json/GetEventDetails?EventID=" + id + "&_fields=Description");
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status != 200) return;
		result = JSONparse(result);
		var description = "";
...
xignite.fetchAnalystRecommendations = function (symbolList, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchAnalystRecommendations.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}

	var url = xignite.makeUrl("/factsetestimates_xignite/xFactSetEstimates.json/GetLatestRecommendationSummaries?IdentifierType=Symbol&UpdatedSince=&Identifiers=" + encodeURIComponent(_.join(symbolList)));
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status != 200) {
			cb("Data Source Error", {}, extraParams);
			return;
		}
		result = JSON.parse(result);
		if (!result.length) {
			cb("No Recommendations Available", {}, extraParams);
			return;
		}
		var items = {};
		for (var s = 0; s < result.length; s++) {
			var item = result[s].RecommendationSummarySet[0];
			item.Company = result[s].Company;
			var dateParts = item.Date.split("/");
			item.Date = new Date(dateParts[2], dateParts[0] - 1, dateParts[1], 0, 0, 0, 0);
			//item.Subject = PortalCore.entityDecode(item.Title);
			item.Recommendations = [];
			item.Recommendations.push(['Buy', item.NumberOfBuyRecommendations]);
			item.Recommendations.push(['Overweight', item.NumberOfOverweightRecommendations]);
			item.Recommendations.push(['Hold', item.NumberOfHoldRecommendations]);
			item.Recommendations.push(['Underweight', item.NumberOfUnderweightRecommendations]);
			item.Recommendations.push(['Sell', item.NumberOfSellRecommendations]);
			items[item.Company.Symbol] = item;
		}
		cb(null, items, extraParams);
	});
...
xignite.fetchAnalystEstimates = function (symbolList, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchAnalystEstimates.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}

	var url = xignite.makeUrl("/factsetestimates_xignite/xFactSetEstimates.json/GetLatestEstimates?IdentifierType=Symbol&EstimateTypes=DividendsPerShare,EPS,EPSReported,ForwardDividendYield,ForwardPERatio,Sales&ReportType=Annual&Identifiers=" + encodeURIComponent(_.join(symbolList)) + "&UpdatedSince=");
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status != 200) {
			cb("Data Source Error", null, extraParams);
			return;
		...
		result = JSON.parse(result);
		if (!result.EstimatesSets) {
			cb("No Data", null, extraParams);
			return;
		}
		var items = {}
		for (var s = 0; s < result.EstimatesSets.length; s++) {
			var estimate = result.EstimatesSets[s];
			if (!items[estimate.EstimateFiscalPeriod]) items[estimate.EstimateFiscalPeriod] = {};
			items[estimate.EstimateFiscalPeriod][estimate.EstimateType] = estimate.Estimates;
		}
		cb(null, items, extraParams);
	});
...
xignite.fetchInstitutionalHolders = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchInstitutionalHolders.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}

	var url = xignite.makeUrl("/www_xignite/xHoldings.json/GetTopHolders?Count=10&IdentifierType=Symbol&Identifier=" + encodeURIComponent(settings.symbol));
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status != 200) {
			cb("Data Source Error", null, extraParams);
			return;
		...
		var count = 0;
		var items = [];
		for (var s = 0; s < result.Holders.length; s++) {
			var item = result.Holders[s];
			item.HolderName = item.Holder.Name;
			item.SharesHeld = item.Holding.Shares;
			item.SharesValue = item.Holding.Value;
			items.push(item);
		}
		cb(null, items, extraParams);
	});
...
xignite.fetchInsiderHolders = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchInstitutionalHolders.apply(self, args);
	...
	var url = xignite.makeUrl("/www_xignite/xInsider.json/GetIssuerOwnerships?IssuerIdentifierType=Symbol&IssuerIdentifier=" + encodeURIComponent(settings.symbol));
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status != 200) {
			cb("Data Source Error", null, extraParams);
			return;
		...
		var count = 0;
		var items = [];
		for (var s = 0; s < result.Ownerships.length; s++) {
			var item = result.Ownerships[s];
			item.HolderName = item.Insider.Name;
			item.HolderRelationship = item.Relationship.IsDirector ? 'Director' : (item.Relationship.IsOfficer ? item.Relationship.OfficerTitle : 'Other');
			items.push(item);
		}
		cb(null, items, extraParams);
...
xignite.fetchOptionChain = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchOptionChain.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}

	var url = xignite.makeUrl("/globaloptions_xignite/xglobaloptions.json/GetAllEquityOptionChain?OptionExchange=&SymbologyType=ListingSymbol&IdentifierType=Symbol&Identifier=" + encodeURIComponent(settings.symbol));
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status != 200) {
			cb("Data Source Error", null, extraParams);
			return;
		...
		var count = 0;
		var items = [];
		for (var s = 0; s < result.Expirations.length; s++) {
			var item = result.Expirations[s];
			//item.HolderName = item.Insider.Name;
			//item.HolderRelationship = item.Relationship.IsDirector ? 'Director' : (item.Relationship.IsOfficer ? item.Relationship.OfficerTitle: 'Other');
			items.push(item);
		}
		if (!window.cachedQuotes) window.cachedQuotes = {};
		if (!window.cachedQuotes[settings.symbol] && result.Quote.Last) window.cachedQuotes[settings.symbol] = result.Quote;
		cb(null, items, extraParams);

	});
}

xignite.fetchCurrencyList = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchCurrencyList.apply(self, args);
			};
		}(this, arguments), 100);
		return;
	}

	var url = xignite.makeUrl("/globalcurrencies_xignite/xGlobalCurrencies.json/ListActiveCurrencies");
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status != 200) {
			cb("Data Source Error", null, extraParams);
			return;
		...
		result = JSON.parse(result);
		cb(null, result.CurrencyList, extraParams);
...
xignite.fetchCurrencyList2 = function (settings, cb, extraParams) {
	if (!xignite.setXigniteEncryptedToken(encryptedToken, false, true)) {
		setTimeout(function (self, args) {
			return function () {
				xignite.fetchCurrencyList.apply(self, args);
	...
	var url = xignite.makeUrl("/globalcurrencies_xignite/xGlobalCurrencies.json/GetAllRealTimeRates?Symbols=USD");
	PortalCore.serverFetch(url, null, null, function (status, result) {
		if (status != 200) {
			cb("Data Source Error", null, extraParams);
			return;
