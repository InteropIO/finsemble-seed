//require(['modules/xignite'], function () {
if (!window.createMarketFactory) window.createMarketFactory = {};
window.createMarketFactory.xignite = function () {

	var jsonToken = localStorage.getItem('Xignite.token');
	var token = JSON.parse(jsonToken);

	CIQ.QuoteFeed.Xignite.Utility.overrides.token = token.token;
	CIQ.QuoteFeed.Xignite.Utility.overrides.tokenUser = token.userid;
	...
	if (!portalSettings.dataSourceSettings || !portalSettings.dataSourceSettings.BATS) {
		CIQ.QuoteFeed.Xignite.prototype.isBats = function () {
			return false;
		}
	}

	CIQ.QuoteFeed.Xignite.prototype.fetch2 = CIQ.QuoteFeed.Xignite.prototype.fetch;

	CIQ.QuoteFeed.Xignite.prototype.fetch = function (params, cb) {
		if (typeof params.symbol == 'object' && params.symbol !== null) {
			params.symbolObject = params.symbol;;
			params.symbol = params.symbolObject.symbol;
		}

		if (params.symbolObject && params.symbolObject.fundamental) {
			if (!CIQ.QuoteFeed.Xignite.CachedFundamentals) CIQ.QuoteFeed.Xignite.CachedFundamentals = {};

			var cacheKey = params.symbolObject.mainSymbol + '_' + params.symbolObject.fundamental.fundamental + '_' + params.symbolObject.fundamental.term;

			var fcb = function (err, data) {
				if (data && data.fundamentals) {
					CIQ.QuoteFeed.Xignite.CachedFundamentals[cacheKey] = data;
					var fundamentals = [];
					var settings = data.settings;
					_.each(_.sortBy(data.fundamentals, 'Date'), function (fundamental) {
						fundamentals.push({
							DT: new Date(fundamental.Date),
							Close: fundamental[settings.fundamentals[0].label]
						})
					});

					var availableFundamental = settings.fundamentals[0].term + settings.fundamentals[0].fundamental;

					if (typeof CIQ.Fundamentals.availableFundamentals[availableFundamental].calculate == 'function') {
						fundamentals = CIQ.Fundamentals.availableFundamentals[availableFundamental].calculate(stx.chart.masterData, fundamentals);
					}
				}

				var fundamentalsInDateRange = [];
				...
				if (!params.startDate) {
					...
					switch (params.interval) {
						...
						case "minute":
							...
							startDate = new Date(params.endDate.getTime() - (params.ticks * msBack * 3)); //8 hour day approx requires
							break;
						case "hour":
							msBack = 3600000 * params.period;
							startDate = new Date(params.endDate.getTime() - (params.ticks * msBack * 3));
							break;
						case "day":
							startDate = new Date(params.endDate.getTime());
				...
				_.each(fundamentals, function (fundamental) {
					if (fundamental.DT.getTime() >= masterData[0].DT.getTime() && fundamental.DT.getTime() <= masterData[masterData.length - 1].DT.getTime()) {
						fundamentalsInDateRange.push(fundamental);
					}
				});

				cb({ quotes: fundamentalsInDateRange, moreAvailable: true })
			}


			if (CIQ.QuoteFeed.Xignite.CachedFundamentals[cacheKey]) {
				fcb(null, CIQ.QuoteFeed.Xignite.CachedFundamentals[cacheKey])
			} else {
				var fSettings = {
					symbol: params.symbolObject.mainSymbol,
					fundamentals: [
						params.symbolObject.fundamental
					],
					years: 5
				}
				window.dataSources[portalSettings.dataSource].fetchFundamentalsFiscalRange(fSettings, fcb)
			}
			...
			/*require(['quoteFeedSamples'], function() {
				var q=new CIQ.QuoteFeed.Demo();
				q.market = this.stxx.chart.market;
				if (!params.endDate) params.endDate=new Date();
				q.fetchFromSource(params, cb)
				q.fetchFromSource(params, cb)

			})*/
		} else {
			this.fetch2(params, cb)
		}
	}
...
*/
	...
	window.dataSources['xignite'].marketFactory = function (symbolObject) {
		var symbol = symbolObject.symbol;
		var market = CIQ.Market.NYSE
		if (CIQ.Market[symbol]) market = CIQ.Market[symbol];
		var splitSymbol = symbol.split('.')[1];
		if (splitSymbol && CIQ.Market[splitSymbol]) market = CIQ.Market[splitSymbol];
		//if (CIQ.Market.Symbology.isForeignSymbol(symbol))  market =  null; // 24 hour market definition
		else if (CIQ.Market.Symbology.isFuturesSymbol(symbol)) market = CIQ.Market.GLOBEX;
		else if (CIQ.Market.Symbology.isForexMetal(symbol)) market = CIQ.Market.METALS;
		else if (CIQ.Market.Symbology.isForexSymbol(symbol)) market = CIQ.Market.FOREX;

		//this.setTimeZone(null, market.market_tz);
		return market;
	};
