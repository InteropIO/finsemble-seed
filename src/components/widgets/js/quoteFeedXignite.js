...
/*(function (definition) {
			...
			var mamaCallbackFunction=function(status, res){
				...
				function processData(quotes, params){
					...
					for(i=1;i<arr.length;i++){
						...
						var adjClose=close;
						if(ratio!=1){  // minimize the "fixing" by limiting it to when there is a real ratio
							close=Number((close*ratio).toFixed(8));
							open=Number((open*ratio).toFixed(8));
							high=Number((high*ratio).toFixed(8));
							low=Number((low*ratio).toFixed(8));
						}
					...
					if(isDaily) {
						...
						if(!params.update){
							...
							if (!results.length || CIQ.yyyymmddhhmm(new Date(+myDate)).substr(0, 8) != CIQ.yyyymmddhhmm(results[results.length - 1].DT).substr(0, 8)) {
								if (this.isIndex(symbol)) params.noUpdate = true;
						...
						if(!params.update && !params.noUpdate){
							...
							if (!this.isIndex(symbol)) todayBarFetch=function(s){
								return function(p,c){
									setTimeout(function(){ s.fetch(p,c); },10);
								};
							}(this);
	...
	CIQ.QuoteFeed.Xignite.Templates={
		...
		HistoricalEquity: {
			...
			statics:"IdentifierAsOfDate=&AdjustmentMethod=PriceReturn&_sort=ascending", //&AdjustmentMethod=PriceReturn
			fields:	"HistoricalQuotes.Date,HistoricalQuotes.Close,HistoricalQuotes.Open,HistoricalQuotes.High,HistoricalQuotes.Low,HistoricalQuotes.Volume,HistoricalQuotes.CumulativeAdjustmentFactor",
			results:{
	...
	CIQ.QuoteFeed.Xignite.getSnapshotQuote=function(params, symbol, isDaily, offset, cb, caller){
		...
		if(params.update &&
			...
			!CIQ.ChartEngine.chartShowsHighs(params.stx.layout.chartType)*/){
			var api={
				host: {
				      protocol: "https://",
