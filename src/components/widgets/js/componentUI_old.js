...
//TODO replace CIQ.installTapEvent with jquery.stxtap()
...
(function (definition) {
	...
	console.log("componentUI.js",_exports);
	...
	CIQ.UI.Context.prototype.closeMenu=function(node){
	 ...
	 *
	 * If you're using your own menuing system then do not call this method. Instead follow the same logic by calling {@link CIQ.ChartEngine#modalBegin}
	 * and {@link CIQ.ChartEngine#modalEnd} when your menuing system is open or closed
	 *
	 ...
	 * @memberOf CIQ.UI.Context
	 */
	CIQ.UI.Context.prototype.startMenuProcessing=function(params){
		if(!params) this.menuParams=params={};
		else this.menuParams=CIQ.clone(params);
		...
		var noCloseSelector=this.menuParams.noCloseSelector=CIQ.UI.Selectors.noClose;

		var context=this;
		context.activeMenuCount=0;
		context.activeDialogCount=0; // Note used by CIQ.UI.Dialog because the overlay blocks any possible action, but available for homespun dialogs
	 ...
	 * @param {String} node 
	 * @param {HTMLElement} The menu to display or selector for that menu.
	 * @memberOf CIQ.UI.Context
	 */
	CIQ.UI.Context.prototype.activateMenu=function(node, closeExistingMenus){
		var self = $(node);
		var activeClassName=this.menuParams.activeClassName;
		var selector=this.menuParams.selector;
		var context=this;
	...
	CIQ.UI.Context.prototype.processBodyTapClaims=function(e){
		var claims=this.topNode.CIQ.UI.EventClaims;
		for(var i=claims.length-1;i>-1;i--){
			if(claims[i].eventType!==CIQ.UI.BODYTAP) continue;
			var helper=claims[i].helper;
			if(helper.bodyTap(e)){
				e.preventDefault();
				return true;
	 ...
	 * @param  {CIQ.UI.Helper} helper The helper to register
	 * @memberOf CIQ.UI.Context
	 */
	CIQ.UI.Context.prototype.registerForResize=function(helper){
		this.registeredResizes.push(helper);
	};

	/**
	 * If a helper needs to be destroyed, unregister it here
	 * @param  {CIQ.UI.Helper} helper The helper to unregister
	 * @memberOf CIQ.UI.Context
	 */
	CIQ.UI.Context.prototype.unregister=function(helper){
	 ...
	 * @memberOf CIQ.UI.Context
	 */
	CIQ.UI.Context.prototype.resize=function(){
		var self=this;
	...
	CIQ.UI.Context.prototype.restoreLifts=function(){
	 ...
	 * @private
	 */
	CIQ.UI.Context.prototype.lift=function(node){
	 ...
	 */
	CIQ.UI.ContextTag.containerExecute=function(fn, args){
		var myArgs=args;
	...
	CIQ.UI.StudyMenu.prototype.renderMenu=function(){
		...
		for(var i=0;i<alphabetized.length;i++){
			...
			menuItem.on("stxtap", menuItem[0].selectFC);
		}
	};
	...
	CIQ.UI.StudyMenu.prototype.studyDialog=function(params){
		...
		if(!studyDialog.length){
			console.log("CIQ.UI.StudyEdit.prototype.editPanel: no StudyDialog available");
			return;
		}
	...
	CIQ.UI.ViewsMenu.prototype.renderMenu=function(){
		...
		if(addNew){
			addNew.stxtap(function(e){
				...
				input.val("");
				viewDialog.parents("cq-dialog")[0].show();
			});
		}
	...
	CIQ.UI.StudyEdit.prototype.editOverlay=function(params){
		...
		if(params.forceEdit){
			...
			this.contextDialog.show({x:CIQ.ChartEngine.crosshairX, y:CIQ.ChartEngine.crosshairY});
		}
	};
	...
	CIQ.UI.Prototypes.Scroll=Object.create(CIQ.UI.ContextTag);

	/**
	 * Scroll back to top
	 * @return {type} [description] as an array
	 */
	...
	CIQ.UI.Prototypes.Scroll.createdCallback=function(){
		...
		if(this.node.perfectScrollbar) this.node.perfectScrollbar({suppressScrollX:true});
	...
	CIQ.UI.Prototypes.Menu=Object.create(CIQ.UI.Prototypes.Scroll);
	...
	CIQ.UI.Prototypes.DrawingToolbar.defaultElements={
		...
		"crossline":["cq-line-color", "cq-line-color", "cq-axis-label", "cq-magnetize"],
		"continuous":["cq-line-style", "cq-line-color", "cq-magnetize"],
		"fibonacci":["cq-line-color", "cq-fill-color", "cq-magnetize"],
		"fibarc":["cq-line-color", "cq-fill-color", "cq-magnetize"],
		"fibfan":["cq-line-color", "cq-fill-color", "cq-magnetize"],
		"fibtimezone":["cq-line-color", "cq-fill-color", "cq-magnetize"],
		"freeform":["cq-line-style", "cq-line-color"],
		"pitchfork":["cq-line-style", "cq-line-color", "cq-magnetize"],
		"regression":["cq-line-style", "cq-line-color", "cq-magnetize"],
		"annotation":["cq-line-color", "cq-annotation", "cq-magnetize"],
		"callout":["cq-line-style", "cq-line-color", "cq-fill-color", "cq-annotation", "cq-magnetize"],
		...
		"speedline":["cq-line-style", "cq-line-color", "cq-fill-color", "cq-magnetize"],
		"speedarc":["cq-line-style", "cq-line-color", "cq-fill-color", "cq-magnetize"],
		"gannfan":["cq-line-style", "cq-line-color", "cq-fill-color", "cq-magnetize"],
		"timecycle":["cq-line-style", "cq-line-color", "cq-fill-color", "cq-magnetize"],
		"quadrant":["cq-line-style", "cq-line-color", "cq-fill-color", "cq-magnetize"],
		"tirone":["cq-line-style", "cq-line-color", "cq-fill-color", "cq-magnetize"],
	};
	...
	CIQ.UI.Prototypes.Dialog.show=function(params){
		...
		if(!this.context.overlay){
			...
			if(!this.params.requireExplicitClose) CIQ.safeClickTouch(this.context.overlay[0], hide());
		}
	 ...
	 */
	CIQ.UI.Prototypes.Swatch=Object.create(CIQ.UI.ContextTag);
	...
	CIQ.UI.Prototypes.Swatch.launchColorPicker=function(){
		...
		if(!colorPickers.length){
			console.log("CIQ.UI.Prototypes.Swatch.launchColorPicker: no ColorPicker available");
			return;
		}
	 ...
	 */
	CIQ.UI.Prototypes.ColorPicker=Object.create(CIQ.UI.ContextTag);
	...
	CIQ.UI.Prototypes.StudyDialog.configure=function(params){
		...
		function makeMenu(name, currentValue, fields){
			...
			var cqMenu=menu.find("cq-menu"); // scrollable in menu.
	 ...
	 * @constructor 
	 */
	CIQ.UI.Prototypes.StudyLegend=Object.create(CIQ.UI.ContextTag);
	...
	CIQ.UI.Prototypes.StudyLegend.renderLegend=function(){
		...
		for(id in stx.layout.studies){
			...
			if(overlaysOnly && !sd.study.overlay) continue;
	...
	CIQ.UI.Prototypes.Themes.configureMenu=function(){
		...
		for(var className in builtInThemes){
			...
			newMenuItem.on("stxtap", newMenuItem[0].selectFC);
	...
	CIQ.UI.Prototypes.Lookup.keyStroke=function(hub, key, e, keystroke){
		...
		var iAmActive=false;
		if(domChain.hasClass(this.context.menuParams.activeClassName)) iAmActive=true; // if my menu chain is active then I'm alive
		if(focused) iAmActive=true; // If my input is focused, then I'm alice
		if(!iAmActive){ // Otherwise, I may or may not be alive
			if(typeof(this.node.attr("cq-keystroke-default"))=="undefined") return; // I'm only alive if I had default body keystrokes, but even then...
			if(this.context.isModal()) return; // If a dialog is open (but I'm not focused ^^) I'm not alive
			if(this.context.activeMenuCount) return; // If a menu is open (but not mine ^^) then I'm not alive
		}
			...
			if(input.value.length){
				...
				if(window.getSelection().toString()){
					...
					setTimeout(function(){
						...
						//if(value=="") CIQ.blur(input);
						self.acceptText(value, self.currentFilter);
					},0);
		...
		if(result){
			...
			setTimeout(function(){
				if(input.value.length/* && !this.usingEmptyDriver*/){
					self.context.activateMenu($(self), true);
				}else{
					self.context.closeMenu(self);
				}
			},0);
	...
	CIQ.UI.Prototypes.Lookup.results=function(arr){
		...
		for(var i=0;i<arr.length;i++){
			...
			node.on("stxtap", node[0].selectFC);
		}
		var scrollable=this.node.find("cq-scroll");
