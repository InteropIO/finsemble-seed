...
(function (definition) {
	...
	console.log("addOns.js",_exports);
	...
	CIQ.Tooltip=function(tooltipParams){
		...
		var node=$("stx-hu-tooltip")[0];
		if(!node){
			node=$("<stx-hu-tooltip></stx-hu-tooltip>").appendTo($('body'))[0];
		}
			...
			fields.forEach(function(name){
				if((this.chart.dataSegment[bar][name] || this.chart.dataSegment[bar][name]===0) && 
					...
					if(this.chart.dataSegment[bar][name].constructor==Number){
						fieldValue=Math.round(this.chart.dataSegment[bar][name]*10000000)/10000000;
