...
(function(_exports, market){
	...
	var CIQ=_exports.CIQ,
		 ...
		 * Set to null to have the justification automatically adjusted based on the axis position.
		 *
		 ...
		 * @memberof CIQ.ChartEngine.YAxis
		 * @since  15-07-01
		 * @since 6.1.2 formalized distinction between null and false values
		 */
		...
		CIQ.extend(CIQ.ChartEngine.Chart.prototype, {
			    ...
			    /**
			     * Set to true to make a comparison series force a percent chart scale.
			     * If true, as long as a comparison series exists on the chart, the chart will have a percent scale.
			     * Once the last comparison series is removed from the chart, the chart will be set to linear scale.
			     * Note this will only take effect on the main chart panel's main axis.
			     * If false, the chart will not change scale when comparison series is added.
				 *
			     * @type boolean
			     * @default
			     * @memberof CIQ.ChartEngine.Chart#
	...
	*/
	 ...
	 * <br>&bull; TBD added internationalize argument
	 */
	 ...
	 * <br>&bull; TBD `isComparison` will put the chart's y-axis into percent scale only if the chart's forcePercentComparison property is true.
	 */
	 ...
	 * @param {boolean} [params.fromTrade] True when called from a function which updates a trade, such as updateTradeData.
	 * @memberof CIQ.ChartEngine
	 * @since 6.1.0
	 * @since TBD Added param fromTrade
	*/
	CIQ.ChartEngine.prototype.updateCurrentMarketData=function(data, chart, symbol, params){
		...
		if(data.Close && (!sq.Last || sq.Last.DT<=data.DT)){
			if(!sq.Last) sq.Last={};
			if(!sq.animatedLast) {  // if this is set elsewhere (animation) then don't worry, Last is also set
				sq.Last.DT=data.DT;
				sq.Last.Price=data.Close;
			}
		}
	...
	*/
	...
	CIQ.ChartEngine.prototype.calculateYAxisPositions=function(){
 	...
 	/**
	 * This method determines and returns the existing position of a y-axis (left or right), regardless of the setting of the `position` property.
	...
	CIQ.ChartEngine.prototype.getYAxisCurrentPosition=function(yAxis, panel){
 	...
 	/**
	 * Sets the yAxis position and recalculates the positions.
	 * Use this method rather than setting the yAxis.position property.
	 * @param {CIQ.ChartEngine.YAxis} yAxis The YAxis whose position is to be set
	 * @param {string} [position] The position ("left", "right", "none", or null)
	 *
	 * @memberof CIQ.ChartEngine
	 * @since 6.1.2
	 */
