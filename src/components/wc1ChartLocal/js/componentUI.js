...
(function (definition) {
	...
	var claims=[];
	var _HTMLElement=_exports.HTMLElement
	...
	class BaseComponent extends HTMLElement {
		...
		static attachedCallback() {
			BaseComponent.prototype.connectedCallback.apply(this)
		}
		...
		connectedCallback(){
			...
			BaseComponent.buildReverseBindings.apply(this);
			this.attached=true;
		};
		...
		scheduleForBinding(node){
			this.scheduledBindings.push({node:node, contextTag:this});
	...
	// var BaseComponent=CIQ.UI.BaseComponent=function(){
	// 	constructor=function(){
	// 		this.ciqInheritsFrom(HTMLElement);
	// 	}
	// };
	...
	class ContextTag extends BaseComponent {
		...
		static attachedCallback(){
			ContextTag.prototype.connectedCallback.apply(this)
		}
	 ...
	 * @memberof CIQ.UI
	 */
	CIQ.UI.ModalTag = class ModalTag extends ContextTag {
		constructor() {
			super()
		}
	...
	DialogContentTag.connectedCallback=function(){
		BaseComponent.connectedCallback.apply(this);
	};
	...
	DialogContentTag.disconnectedCallback=function(){
...
function (_HTMLElement) {
  ...
  function TestComponent() {
    ...
    return _possibleConstructorReturn(this, _getPrototypeOf(TestComponent).call(this));
  }

  return TestComponent;
}(_wrapNativeSuper(_HTMLElement));


customElements.define('test-component', TestComponent);

	var UIManager = function(_HTMLElement) {
		_inherits(UIManager, _HTMLElement)

		function UIManager () {
		var self= _possibleConstructorReturn(this, _getPrototypeOf(UIManager).call(this))
		self.activeMenuStack=[],
		self.registeredForResize=[],
		self.keystrokehub=null
	/**
	 ...
	 * @param {object} params
	 */
	self.openMenu=function(menu, params){
	 ...
	 * @alias registerForResize
 	 * @param {HTMLElement} element
	 */
	self.registerForResize=function(element){
		self.registeredForResize.push(element);
	};
	 ...
	 * @param {HTMLElement} element
	 */
	self.unregisterForResize=function(element){
	 ...
	 * @param {HTMLElement} element
	 */
	self.restoreLift=function(element){
		var node=$(element);
	...
	// UIManager.ciqInheritsFrom(HTMLElement.prototype)
	CIQ.UI.UIManger=UIManager;

	customElements.define("cq-ui-manager", UIManager);
	// customElements.define("cq-um", UM)
	return _exports;
});
