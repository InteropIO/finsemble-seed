(function(CIQ) {
	...
	var qs = {
		...
		stringify: function(obj, sep, eq) {
			...
			return Object.keys(obj).map(function(k) {
				if (Array.isArray(obj[k])) {
					return obj[k].map(function(v) { return [k, typeof v === 'object' ? '' : v].map(window.encodeURIComponent).join(eq); }).join(sep);
				}

				return [k, typeof obj[k] === 'object' ? '' : obj[k]].map(window.encodeURIComponent).join(eq);
			}).join(sep);
		},
		...
		parse: function(str, sep, eq) {
			...
			return str.split(sep).reduce(function(memo, part) {
				var kv = part.replace(/\+/g, '%20').split(eq).map(window.decodeURIComponent),
				  key = kv[0],
				  value = kv[1] || '';
	...
	function request(url, query, callback) {
		...
		xhr.addEventListener('load', function() {
			...
			var error = xmlDoc.evaluate('/errors/error_message', xmlDoc, nsResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	 ...
	 */
	CIQ.TradingCentral = function(stx, token, partner) {
		if (!token) throw new Error('CIQ.TradingCentral: No token provided');
		if (!partner) throw new Error('CIQ.TradingCentral: No partner number provided');
		...
		/**
		 * setPeriodicityV2 interval values with TradingCentral term as the key.
		 */
		this.interval = {
			Intraday: 30,
			ST: 'day',
	 ...
	 */
	CIQ.TradingCentral.prototype.analysis = function(options, callback) {
	 ...
	 */
	CIQ.TradingCentral.parse = function(xmlDoc) {
		var nsResolver = xmlDoc.createNSResolver(xmlDoc.documentElement);
		var getElementByXPath = function(path) {
			return xmlDoc.evaluate(path, xmlDoc, nsResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
		};
		var story = getElementByXPath('/items/article/analysis/content/story');
		var result = {
			header: {
				date: getElementByXPath('/items/article/analysis/content/header/date').textContent,
				hour: getElementByXPath('/items/article/analysis/content/header/hour').textContent,
				/**
				...
				get $date() {
					...
					var isoString = year + '-' + month + '-' + day + 'T' + this.hour + ':00.000Z';
				...
				get $age() {
					...
					return Math.floor(age / 1440) + ' days, ' + Math.floor((age % 1440) / 60) + ' hr ago';
				},
				term: getElementByXPath('/items/article/analysis/content/header/term').textContent,
				option: {
					chartlevels: {
						resistance3: getElementByXPath('/items/article/analysis/content/header/option/chartlevels/resistance3').textContent,
						resistance2: getElementByXPath('/items/article/analysis/content/header/option/chartlevels/resistance2').textContent,
						resistance1: getElementByXPath('/items/article/analysis/content/header/option/chartlevels/resistance1').textContent,
						pivot: getElementByXPath('/items/article/analysis/content/header/option/chartlevels/pivot').textContent,
						support1: getElementByXPath('/items/article/analysis/content/header/option/chartlevels/support1').textContent,
						support2: getElementByXPath('/items/article/analysis/content/header/option/chartlevels/support2').textContent,
						support3: getElementByXPath('/items/article/analysis/content/header/option/chartlevels/support3').textContent
					},
					watch: {
						opinionIntraday: getElementByXPath('/items/article/analysis/content/header/option/watch[@type="opinionIntraday"]').textContent,
						opinionST: getElementByXPath('/items/article/analysis/content/header/option/watch[@type="opinionST"]').textContent,
						opinionMT: getElementByXPath('/items/article/analysis/content/header/option/watch[@type="opinionMT"]').textContent,
						deltaST: getElementByXPath('/items/article/analysis/content/header/option/watch[@type="deltaST"]').textContent,
						deltaMT: getElementByXPath('/items/article/analysis/content/header/option/watch[@type="deltaMT"]').textContent
					}
				},
				get directionArrow() {
					var key = ({ST: 'opinionST', MT: 'opinionMT', INTRADAY: 'opinionIntraday'})[this.term];
					return this.option.watch[key];
				},
				get directionName() {
					return this.directionArrow > 0 ? 'bullish' : 'bearish';
				}
			},
			story: {
				keywords: getElementByXPath('/items/article/analysis/content/story/keywords').textContent,
				title: getElementByXPath('/items/article/analysis/content/story/title').textContent,
				summary: getElementByXPath('/items/article/analysis/content/story/summary').textContent,
				subsections: Array.prototype.map
					.call(story.children || story.childNodes, function(element, index, arr) {
					...
					.reduce(function(memo, end, index, arr) {
						...
						var section = Array.prototype.slice.call(story.children || story.childNodes, start, end)
							.reduce(function(m, e) {
	 ...
	 */
	CIQ.TradingCentral.prototype.hideIndicators = function() {
		if (this.rsi) {
	 ...
	 */
	CIQ.TradingCentral.prototype.showIndicators = function() {
		if (!this.rsi) {
			this.rsi = CIQ.Studies.addStudy(this.stx, "rsi", {"Period": 14}, {"RSI": "#8ec648"});
		}

		if (!this.bollingerBands) {
			this.bollingerBands = CIQ.Studies.addStudy(this.stx, "Bollinger Bands", {
				"Field": "field",
				"Period": 20,
	 ...
	 * @private
	 */
	CIQ.TradingCentral.prototype.getY = function(price) {
		var y = Math.floor(this.stx.pixelFromPriceTransform(price, this.panel) + 0.5) + 0.5;
	 ...
	 */
	CIQ.TradingCentral.prototype.priceLine = function(price, colorName, emphasize) {
		var y = this.getY(price);
		if (y === null) return;
		this.stx.plotLine(this.panel.left, this.panel.right, y, y, this.colors[colorName], 'line', this.panel.chart.context, this.panel, {
			pattern: 'solid',
	 ...
	 */
	CIQ.TradingCentral.prototype.priceLabel = function(price, colorName) {
		var y = this.getY(price);
		if (y === null) return;
		this.stx.createYAxisLabel(this.panel, price, y, this.colors[colorName], '#ffffff');
	};

	/**
	 * Calls CIQ.ChartEngine.prototype.removeInjection for each display function
	 */
	CIQ.TradingCentral.prototype.removeInjections = function() {
	 ...
	 */
	CIQ.TradingCentral.prototype.createDrawInjections = function(levels, direction) {
		var self = this;
		...
		this.injections.drawXAxis = this.stx.append('drawXAxis', function() {
			...
			self.priceLine(levels.resistance3, 'resistance', direction > 0 && current > levels.resistance2 && current < levels.resistance3);
			self.priceLine(levels.resistance2, 'resistance', direction > 0 && current > levels.resistance1 && current < levels.resistance2);
			if (parseFloat(levels.resistance1) > levels.pivot) {
				self.priceLine(levels.resistance1, 'resistance', direction > 0 && current < levels.resistance1);
			}
			self.priceLine(levels.pivot, 'pivot');
			if (parseFloat(levels.support1) < levels.pivot) {
				self.priceLine(levels.support1, 'support', direction < 0 && current > levels.support1);
			}
			self.priceLine(levels.support2, 'support', direction < 0 && current < levels.support1 && current > levels.support2);
			self.priceLine(levels.support3, 'support', direction < 0 && current < levels.support2 && current > levels.support3);
		});
	 ...
	 * @example
	 * // create injection
	 * tc.createMouseInjections({}, 1, tcElement);
	 *
	 * // use this event to setup hover response
	 * tcElement.addEventListener('linehoverbegin', function(e) {
	 * 	// e.detail.line;
	 * 	// e.detail.lineY;
	 * 	// e.detail.price;
	 * });
	 ...
	 * tcElement.addEventListener('linehoverend', function(e) {
	 * 	// e.detail.line;
	 * });
	 */
	CIQ.TradingCentral.prototype.createMouseInjections = function(levels, trend, node) {
		var getY = CIQ.TradingCentral.prototype.getY.bind(this);
		...
		this.injections.mousemove = this.stx.append('mousemove', function() {
			...
			if (!eventParams) {
				...
				if (lineHoverCurrent) {
					...
					node.dispatchEvent(new CustomEvent('linehoverend', {detail: eventParams}));
				}
			...
			if (lineHoverCurrent) {
				node.dispatchEvent(new CustomEvent('linehoverend', {detail: {line: lineHoverCurrent}}));
			}

			// done in a zero timeout to ensure 'linehoverend' happens before 'linehoverbegin'
			window.setTimeout(function() {
				lineHoverCurrent = eventParams.line;
				node.dispatchEvent(new CustomEvent('linehoverbegin', {detail: eventParams}));
			});
		});
	};
	 ...
	 */
	CIQ.TradingCentral.prototype.getCurrentTerm = function() {
		var termMap = {
			30: 'Intraday',
			day: 'ST',
			week: 'MT'
