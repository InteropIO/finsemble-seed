...
(function(_exports) {
	...
	Object.defineProperties(CIQ.Scripting.State.prototype, {
		...
		'cloneCustomProperties': {
			value: function() {
				...
				// non-emumerable because children should be handled manually, not by CIQ.clone()
				...
				for (var key in this) {
					...
					empty = false;
					clone[key] = typeof this[key] === 'object' ? CIQ.shallowClone(this[key]) : this[key];
				}

				for (var cKey in this.children) {
					clone.children[cKey] = this.children[cKey].cloneCustomProperties();
					empty = empty && clone.children[cKey] === null;
				}
	 ...
	 * @memberof CIQ.Scripting
	 * @private
	 * @since 6.1.2 accepts the study descriptor instead of the context object. Now creates the context object and assigns it as the "context" property on the return value.
	 */
