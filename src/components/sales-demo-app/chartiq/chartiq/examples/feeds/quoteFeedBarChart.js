...
CIQ.QuoteFeed.BarChart.prototype.isAfterDelayed = function () {
	var nd = CIQ.getETDateTime();
	if (nd.getHours() > 16 || (nd.getHours() == 16 && nd.getMinutes() > 20))
		return true;
	return false;
};
CIQ.QuoteFeed.BarChart.prototype.fetch = function (params, cb) {
	...
	CIQ.postAjax(url, null, function (status, res) {
		...
		var moreToLoad = !params.stx.quoteDriver.getQuoteFeed(params).behavior
			.noLoadMore;
...
CIQ.QuoteFeed.BarChart.prototype.process = function (quotes, params) {
	...
	var fieldNames = {};
	var suppressVolume =
		params.update && CIQ.ChartEngine.isDailyInterval(interval);
	var i;
	...
	for (i = 1; i < arr.length; i++) {
		...
		newQuotes.push({
			...
			Volume: suppressVolume
				? 0
				: parseFloat(fields[fieldNames.volume].replace(/"/g, "")),
