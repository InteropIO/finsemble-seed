					...
					(function () {
						...
						return function (data) {
							...
							translateAndSendToChart(data, symbol, function (res) {
								...
								if (res.error) {
									// NOT NYSE symbol, go to simulator
									var queryUrl =
										quoteFeedSunGard.quoteFeedSimulator.url +
										"?session=" +
										params.quoteDriverID + // add on unique sessionID required by ChartIQ simulator;
										"&identifier=" +
										symbol +
										"&startdate=" +
										new Date(suggestedStartDate).toISOString() +
										"&enddate=" +
										new Date(suggestedEndDate).toISOString() +
										"&interval=" +
										params.interval +
										"&period=" +
										params.period +
									...
									quoteFeedSunGard.quoteFeedSimulator.postAjax(queryUrl, function (status, response) {
										// process the HTTP response from the datafeed
										if (status == 200) {
											// if successful response from datafeed

											var newQuotes = quoteFeedSunGard.quoteFeedSimulator.formatChartData(response, symbol);
											const chain = quoteFeedSunGard.addOptionsChain(newQuotes, params);
											console.log(chain)
											cb({
												quotes: newQuotes,
												moreAvailable: true,
...
quoteFeedSunGard.subscribe = function (params) {
	// CIQ.Market.Symbology.factory(params.symbol).then(symbol => {
...
quoteFeedSunGard.getMuse = () => {
	return new Promise((resolve, reject) => {
		quoteFeedSunGard.loadDependencies();
...
quoteFeedSunGard.addOptionsChain = (quotes, params) => {
	console.log('quotes', quotes, params)
	const daily = quoteFeedSunGard.quoteFeedSimulator.isDaily(params);
	let quotesToPopulate = [];

	for (
		let i = 0;
		i < quotes.length && quotesToPopulate.length < quoteFeedSunGard.quoteFeedSimulator.CONSTS.HIST_DEPTH;
		i++
	) {
		const recentQuote = quotes[quotes.length - i - 1];
		if (daily || !quoteFeedSunGard.quoteFeedSimulator.isMarketClosed(recentQuote.DT))
			quotesToPopulate.push(recentQuote);
	}
	while (quotesToPopulate.length) {
		const quote = quotesToPopulate.pop();
		quoteFeedSunGard.quoteFeedSimulator.populateExpirations(quote.DT, quote.Close, params.symbol);
		const factor = quoteFeedSunGard.quoteFeedSimulator.getFractionOfInterval(
			params,
			null,
		...
		if (factor) {
			quote.optionChain = quoteFeedSunGard.quoteFeedSimulator.randomChain(quote, params.symbol, factor);
			if (quoteFeedSunGard.quoteFeedSimulator.CONSTS.DEBUG && !quotesToPopulate.length) quoteFeedSunGard.quoteFeedSimulator.logTables(quote);
		}
	}
}
