 ...
 * This file contains a set of overrides, customised to work with a viewer.js based PDF viewer component, that will 
...
window.open = function (urlToOpen, name, specs, replace) {
	...
	if (name && name.includes("openfin-child-window")) {
		originalWindowOpen.call(window, urlToOpen, name, specs, replace);
		return;
	}
...
const handlePdfUrl = function(urlToOpen, name, specs, replace) {
	...
	if (useLinker && (!name || name !="_blank")) {
		...
		FSBL.Clients.LauncherClient.spawn("viewerJs", { 
			...
			if (err) {
				console.error("viewerJsNativeOverrides.js window.open patch error: " + err);
			} else {
