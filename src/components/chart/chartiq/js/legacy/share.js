...
(function (definition) {
	 ...
	 * context.drawImage(img, 10, stx.chart.panel.height-50, 60,60);  //Parameters are: image. x axis, y axis, width, hight.
	 * ```
	...
	CIQ.Share.brandMyChart=function(stx, imageURL, positioning, size){
		function prependDisplayChart(stx, image, positioning,size){
			return function(){
				...
				var y=stx.chart.panel.height/2-height/2;
				if(positioning){
					if(positioning[0]>0){
						x=positioning[0];
					}else{
					...
					if(positioning[1]>0){
						...
						y=stx.chart.panel.height-height+positioning[1];
					}
				}
				stx.chart.context.drawImage(image, x, stx.chart.panel.top+y, width, height);
			};
		}
