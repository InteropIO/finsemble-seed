...
(function (definition) {
	...
	CIQ.QuoteFeed.CopyAndPasteMe.prototype.fetchFromSource=function(params, cb){
		...
		if(params.startDate && params.endDate){
			...
			// (This is streaming by "polling". For actual push based streaming see {@link CIQ.ChartEngine.appendMasterData}.
			//
	...
	CIQ.QuoteFeed.CopyAndPasteMe.prototype.subscribe=function(params){
		...
		// streamer push updates {@link CIQ.ChartEngine@appendMasterData}.
		//
	 ...
	 * @param {object} params
	 * @param {string} params.inverval Either tick, minute, second, or millisecond
	...
	CIQ.QuoteFeed.Demo.prototype.fetchFromSource=function(params, cb){
		...
		if(params.startDate && params.endDate ){
			...
			return;
		} else if(params.startDate && params.update){
			// new update
			if(params.interval=='tick' || params.interval=="minute" || params.interval=="second" || params.interval=="millisecond"){
	...
	CIQ.QuoteFeed.Demo.prototype.generateIntraday=function(params, cb){
		...
		// enable pre and post market hours to ensure extended hours works properly
		var sessions=["pre","post"];
		for(var s=0; s<sessions.length; s++) {
			this.market.enableSession(sessions[s]);
		}
		...
		var iter = this.market.newIterator(
				{
					...
					'periodicity': 1, //params.stx.layout.periodicity, // always do 1 since this is the raw data. The aggregation will happen upon data returned.
	...
	CIQ.QuoteFeed.Demo.prototype.generateIntradayRange=function(params, cb){
		...
		for(var i=0; i<sessions.length; i++) {
			this.market.enableSession(sessions[i]);
		}
	 ...
	 * @param {object} params
	 * @param {*} result Data to pass to callback
	 * @param {function} cb Callback
	 * @memberOf CIQ.QuoteFeed.ChartIQEOD
	 */
