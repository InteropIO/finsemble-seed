(function (definition) {
	...
	/**
	 * ** This function is maintained for legacy implementations only (not using web components). New implementations should use functionality included in the web components **<br>
	 * Adds a new comparison symbol. This method is driven from the UI but can also be called programmatically if the comparison UI is at least
		...
		function processResponse(symbol){
			return function(err){
				...
				if(!stx.comparisons[symbol]){
					...
					if(stx.chart.series[symbol].parameters.permanent) $$$(".stx-ico-close", div).style.display="none";
					else{
