...
const MongoStorageAdapter = function (uuid) {
	...
	this.save = function (params, cb) {
		...
		if (!params.complete) {
			combinedKey = this.getCombinedKey(this, params);
		} else {
			combinedKey = params.key;
		}

		Logger.system.debug("Mongo.save for key=" + combinedKey + " with data=" + params.value);
		console.log("Mongo.save for key=" + combinedKey + " with data=" + params.value);
	...
	this.delete = (params, cb) =>  {
		...
		fetch(`${this.baseURL}/delete?key=${combinedKey}`, {
			method: 'DELETE'
		})
		.then(data => {
			cb(null, { status: "success" });
