...
const runZoomHandler = () => {
	...
	FSBL.System.Window.getCurrent().setZoomLevel = function (level, callback, errorCallback) {
		callback();
	};
	...
	FSBL.Clients.LauncherClient.getComponentDefaultConfig(
		...
		(err, componentConfig) => {
			...
			FSBL.Clients.ConfigClient.get(["finsemble", "Window Manager", "zoom"], zoomConfigHandler);
		}
	);
