...
export function resolveIntent(intent, intentList, activeComponents, context){
	let index = 0;
	let activeMatches = [];
	console.log(intentList.apps);
	let availableTargets = [];
	let activeIds = Object.keys(activeComponents);
	intentList.apps.forEach(function (app){
		console.log("App:", app.name);
		availableTargets.push(app.name);
	});
	console.log(availableTargets);
	Object.values(activeComponents).forEach(function (component){
		console.log(component.componentType);
		if(availableTargets.includes(component.componentType)){
			// debugger;
			let match = {
