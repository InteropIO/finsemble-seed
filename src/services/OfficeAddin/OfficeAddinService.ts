...
export default class OfficeAddinService extends Finsemble.baseService {
	...
	private fileList: Array<ExcelFile>;
	private actionList: Array<{ action: String, ids: [] }>
	...
	createRouterEndpoints() {
		this.RouterClient.addListener("finsemble-excel-event", this.handleExcelEvent)
		setInterval(this.checkFileList, 1000)
	...
	handleExcelEvent: StandardCallback = (err, res) => {
		if (err) {
			this.Logger.error("OfficeAddinService error when handling router message", err)
		} else {
			if (!res.originatedHere()) {
				...
				switch (res.data.event) {
					case 'ADDIN_OPENED':
						let file = this.fileList.find(file => {
							return file.fileName == res.data.fileName
						})

						if (!file) {
							let newExcelfile = new ExcelFile(res.data.fileName, res.data.fileUrl, res.data.timestamp, res.data.timestamp)
							this.fileList.push(newExcelfile)
							Finsemble.Clients.RouterClient.addListener(`${res.data.fileName}-event`, this.handleExcelFileEvent)
						} else {
	...
	query(channel: string, data: {}, channelPrefix?: string, callback?: StandardCallback): Promise<any> {
		return new Promise<{}>(async (resolve, reject) => {
			try {
				this.Logger.system.info(`sending message on ${channelPrefix + channel} channel`, data);
	 ...
	 */
	addResponder(channel: string, dataProcessor: (data: any) => any, channelPrefix?: string) {
		if (channelPrefix == null) {
			channelPrefix = ''
		}

		this.Logger.system.info(`Adding responder for endpoint: ${channelPrefix + channel}`);
		...
		this.RouterClient.addResponder(channelPrefix + channel, (err: any, message: any) => {
			...
			try {
				this.Logger.system.info(`Processing message on channel ${channel}: `, message);
				const returnVal = dataProcessor(message.data);
				this.Logger.system.info(`Message response on ${channel}: `, returnVal);
	...
	checkFileList = () => {
		this.fileList.forEach((file, index, object) => {
			this.query(`${file.fileName}-${file.createTimestamp}`, { action: 'HEALTH_CHECK' }, 'query-')
				.then((res) => {
			...
			if (new Date().getTime() - file.aliveTimestamp >= 2000) {
				...
				this.RouterClient.removeListener(`${file.fileName}-event`, this.handleExcelFileEvent);
