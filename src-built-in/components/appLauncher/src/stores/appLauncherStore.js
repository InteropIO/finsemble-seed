...
var Actions = {
	...
	getComponentList(cb) {
		FSBL.Clients.LauncherClient.getComponentList((err, response) => {
			...
			FSBL.Clients.Logger.debug(
				"getComponentList from LauncherClient",
				response
			);
	...
	getUserDefinedComponentList(cb) {
		...
		FSBL.Clients.StorageClient.get(
			{ topic: "finsemble", key: "userDefinedComponents" },
			(err, response) => {
				var data = response;
				if (data) {
	...
	filterComponents(components) {
		...
		if (settings.mode) {
			...
			keys.forEach((componentType) => {
				...
				var componentMode =
					config.component && config.component.mode
						? config.component.mode
						: "";
				...
				if (componentMode || componentMode === "") {
					...
					let commonModes = componentMode.filter(
						(n) => settings.mode.indexOf(n) !== -1
					);
	...
	togglePin(componentToToggle) {
		var componentType =
			componentToToggle.group || componentToToggle.component.type;
		var fontIcon;
		try {
		...
		if (
			componentToToggle.component &&
			componentToToggle.component.displayName
		) {
		...
		if (wasPinned) {
			...
			ToolbarStore.setValue({
				field: `pins.${componentTypeDotRemove}`,
				value: thePin,
			});
		}
	},
	...
	removeCustomComponent(componentName) {
		...
		FSBL.Clients.LauncherClient.removeUserDefinedComponent(
			...
			(err, response) => {
				...
				var { pins } = appLauncherStore.values;
				var wasPinned = false;
				if (pins) {
					for (var i = 0; i < pins.length; i++) {
						var pin = pins[i];
