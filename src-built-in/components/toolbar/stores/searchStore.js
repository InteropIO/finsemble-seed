...
function mouseInBounds(bounds, cb) {
	FSBL.System.getMousePosition((err, mousePosition) => {
		if (
...
var Actions = {
	...
	setFocus(bool, target) {
				...
				finsembleWindow.getBounds((err, bounds) => {
					if (err)
						FSBL.Clients.Logger.error(
							"setFocus->finsembleWindow.getBounds",
							err
						);
					cachedBounds = bounds;
		...
		menuWindow.isShowing((err, showing) => {
			if (err) {
				FSBL.Clients.Logger.error(`menuWindow.isShowing failed, error:`, err);
			}
			//if (!showing) return//console.log("not showing")
			mouseInWindow(menuWindow, (err, inBounds) => {
				if (err) {
					FSBL.Clients.Logger.error(
						`menuWindow.isShowing->mouseInWindow failed, error:`,
						err
					);
				}
				if (!inBounds) {
			...
			if (showing) {
				...
				if (!e && cachedBounds) {
					finsembleWindow.animate(
						{
							transitions: {
								size: { duration: 150, width: cachedBounds.width },
							},
						},
						() => {
							cachedBounds = null;
	...
	getComponentList(cb) {},
	actionPress(action) {
		menuStore.getValue("list", (err, list) => {
			if (err) {
				FSBL.Clients.Logger.error(
					`Failed to retrieve 'list' value from menuStore, error:`,
					err
				);
			}
			if (!list) return;
			if (list.length > 1) {
				FSBL.Clients.RouterClient.transmit(
					`SearchMenu.${menuWindow.name}.actionpress`,
					action
				);
			}
		});
	...
	search(text) {
		...
		FSBL.Clients.SearchClient.search({ text: text }, (err, response) => {
			if (err) {
				FSBL.Clients.Logger.error(
					`Failed to perform SearchClient.search, error:`,
					err
				);
			}
...
function createStore(done) {
	...
	FSBL.Clients.DistributedStoreClient.createStore(
		{
			store: `Finsemble-SearchStore-${finsembleWindow.name}`,
			values: defaultData,
			global: true,
		},
		(err, store) => {
			...
			store.getValues(["owner", "menuSpawned"], (err, data) => {
				if (err) {
					FSBL.Clients.Logger.error(
				...
				store.addListener(
					{ field: "menuIdentifier" },
					Actions.updateMenuReference
				);
				menuStore.Dispatcher.register((action) => {
				...
				if (!data.menuSpawned) {
					FSBL.Clients.LauncherClient.spawn(
						"searchMenu",
						{
							name: `searchMenu.${finsembleWindow.name}`,
							data: { owner: finsembleWindow.name },
						},
						(err, data) => {
							if (err) {
								FSBL.Clients.Logger.error(
									`LauncherClient.spawn failed for searchMenu, error:`,
									err
								);
							}
					...
					menuStore.getValue("menuIdentifier", (err, menuIdentifier) => {
						if (err) {
							FSBL.Clients.Logger.error(
