...
const menuConfig =
	"menus" in toolbarConfig ? toolbarConfig.menus : toolbarConfig;
...
class _ToolbarStore {
	...
	retrieveSelfFromStorage(cb) {
		FSBL.Clients.StorageClient.get(
			{ topic: finsembleWindow.name, key: finsembleWindow.name },
			(err, result) => {
				if (err || !result) {
				...
				let bounds =
					result && result.hasOwnProperty("window-bounds")
	...
	setToolbarVisibilityInMemory(cb = Function.prototype) {
		...
		FSBL.Clients.StorageClient.save(
			{
				topic: finsembleWindow.name,
				key: finsembleWindow.name,
				value: {
					visible: true,
	...
	loadMenusFromConfig(done, self) {
		FSBL.Clients.ConfigClient.getValue({ field: "finsemble.menus" }, function(
			err,
			menus
		) {
			if (err) {
	...
	setupHotkeys(done) {
		 ...
		 * @param {Object} param
		 * @param {string} param.name the name of the hotkey
		 * @param {Array} param.keyCombination array of keys that make up the hotkey combination
		 * @param {Function} param.action function for the hotkey action
		 * @param {boolean} param.enabled is the hotkey enabled
		 */
		const setHotkey = ({ name, keyCombination, action, enabled }) => {
			try {
					...
					FSBL.Clients.HotkeyClient.addGlobalHotkey(
						...
						() => {
							FSBL.Clients.Logger.log(
								`Hotkey added - ${name} ${keyCombination}`
							);
						}
					);
				} else {
				...
				FSBL.Clients.Logger.error(
					`Failed to add the hotkey ${keyCombination} for ${name}.`,
					err
				);
			}
		};
		 ...
		 *
		 * This function takes a list of hotkeys from the config file and updates / merges the default hotkey list with it's values.
		 * Note that these hotkeys are not assigned in this function and should not be used with set / assigned hotkeys.
		 * @param {Array} configHotkeys
		 * @param {Array} defaultHotkeys
		 */
		const overrideDefaultHotkeysWithConfigHotkeys = (
			configHotkeys,
			defaultHotkeys
		) =>
			defaultHotkeys.map(defaultHotkey => {
				...
				const config = configHotkeys.find(
					configHotkey => defaultHotkey.name === configHotkey.name
				);
		...
		if ("hotkeys" in toolbarConfig) {
			const hotkeys = overrideDefaultHotkeysWithConfigHotkeys(
