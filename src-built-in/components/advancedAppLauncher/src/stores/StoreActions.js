...
function initialize(callback = Function.prototype) {
	FSBL.Clients.ConfigClient.getValue(
		{ field: "finsemble.appDirectoryEndpoint" },
		(err, appDirectoryEndpoint) => {
			...
			if (data.deleted.length > 0) {
				...
				folderList = Object.keys(data.folders)
					.filter((folderName) => !data.deleted.includes(folderName))
					.sort((name1, name2) => {
			...
			data.foldersList =
				folderList || Object.keys(store.values.appFolders.folders);
			data.apps =
				Object.keys(appList).length > 0 ? appList : store.values.appDefinitions;
			...
			store.addListener(
				...
				(err, dt) => (data.folders = dt.value)
			);
			store.addListener(
				{ field: "appFolders.list" },
				(err, dt) => (data.foldersList = dt.value)
			);
			store.addListener(
				{ field: "appDefinitions" },
				(err, dt) => (data.apps = dt.value)
			);
			store.addListener(
				{ field: "activeFolder" },
				(err, dt) => (data.activeFolder = dt.value)
			);
			store.addListener(
				{ field: "isFormVisible" },
				(err, dt) => (data.isFormVisible = dt.value)
			);
			store.addListener(
				{ field: "sortBy" },
				(err, dt) => (data.sortBy = dt.value)
			);
			store.addListener(
				{ field: "activeLauncherTags" },
				(err, dt) => (data.tags = dt.value)
			);
			store.addListener(
				{ field: "deleted" },
				(err, dt) => (data.deleted = dt.value)
			);
...
function updateAppsInFolders(cb = Function.prototype) {
	...
	const {
		ADVANCED_APP_LAUNCHER: advancedAppLauncherFolderName,
	} = getConstants();
			...
			if (component.source && component.source === "FDC3") {
				...
				return getApp(component.appID, (err, app) => {
					if (err) {
						...
						console.error(
							"there was an error loading from FDC3",
							component,
							err
						);
	...
	FSBL.Clients.LauncherClient.getComponentList((err, componentList) => {
		...
		Object.keys(folders).forEach((folderName) => {
			folders[folderName].apps.map((configDefinedApp, i) => {
				const { name } = configDefinedApp;
				const { appID } = configDefinedApp;
		...
		(error, data) => {
			if (error) {
				...
				FSBL.Clients.Logger.error(
					`Advanced App Launcher: Failed to save: ${field}:${value}`
				);
				// TODO
...
function addPin(pin) {
	...
	FSBL.Clients.LauncherClient.getComponentList((err, components) => {
		...
		for (let i = 0; i < Object.keys(components).length; i++) {
			...
			if (
				componentName === pin.name ||
				componentName === `${pin.name} Component`
			) {
		...
		if (componentToToggle) {
			let componentType =
				componentToToggle.group || componentToToggle.component.type || pin.name;
			let fontIcon;
			try {
			...
			if (
				componentToToggle.component &&
				componentToToggle.component.windowGroup
			) {
			...
			var thePin = {
				...
				component: componentToToggle.group
					? componentToToggle.list
					: componentType,
				fontIcon: fontIcon,
				icon: imageIcon,
			...
			ToolbarStore.setValue({
				field: `pins.${pin.name.replace(/[.]/g, "^DOT^")}`,
				value: thePin,
			});
		}
	});
...
function reorderFolders(destIndex, srcIndex) {
	...
	const orderableFolders = data.foldersList.filter(
		(folderName) => !dragDisabled.includes(folderName)
	);
	const movedFolder = orderableFolders[destIndex];
	const remainingItems = orderableFolders.filter(
		(item, index) => index !== destIndex
	);
	data.foldersList = [
		...unorderableFolders,
...
function renameFolder(oldName, newName) {
	...
	_setFolders(() => {
		let indexOfOld = data.foldersList.findIndex(
			(folderName) => folderName === oldName
		);
...
function findAppByField(field, value) {
	return (
		Object.values(data.apps).find((app) =>
			app ? app[field] === value : false
		) ||
		Object.values(data.configComponents).find((app) =>
			app ? app[field] === value : false
		)
	);
}
...
function getAllAppsTags() {
	...
	const apps = Object.values(data.apps).concat(
		Object.values(data.configComponents)
	);
