...
- Muting of notifications is now only possible on sources (originating apps). You can no longer mute based on notification type.

- The notifications preferences panel has been eliminated. Notification settings can now be controlled directly from the notifications panel itself.
...
*Note: running makeInstaller with a zip is still in development*
...
If you are building in a CI environment then you will need to provide the location of your original iocd installer that we provided to you. This may be either an exe or zip file. You may provide either a file path or a url. If we provided an exe, then makeInstaller will generate an actual installer than your end users can run. If we provided a zip, then makeInstaller will generate a folder which you can copy to your end users' desktops. Set the location of your installer in the `iocdInstaller` field in project.json _or_ set an environment variable called IOCDINSTALLER _or_ set IOCDINSTALLER in a .env file at your project's root.
...
If you've configured certificate files and passwords in your project.json then your installer will be signed (or your executable when generating a zip folder). You can also use the new `windowsSign` field which accepts all parameters supported by `@electron/windows-sign`. This gives you access to modern signing technology supported by Microsoft.
