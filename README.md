...
FIN-2011 - Fixed issue where spurious heartbeat "possibly crashed" messages would appear if iocd loses focus. This was caused by some system windows with backgroundThrottling enabled, causing a false alarm.

- Fixed a bug where API calls to FSBL.restartApplication() and FSBL.shutdown() did not prompt the user to save their dirty workspace. Please note that automated (unattended) shutdown & restart will no longer save dirty workspaces to Finsemble storage adapters. Any unsaved changes will be lost.

- Fixed a bug where `unsubscribeCallback` was not invoked for pub-sub responder.

FIN-2095 - Network errors (404, 500, etc) are now logged. They will appear in logs/application/<appName>.log. (requires iocd version 9.9+). These are now forwarded to "logger.service.logMessages" when `iocd.enableLegacyLoggerRecipe` is true in the manifest. Search for "http-error" or "network-error" or the actual http codes (404,500,etc)

- Fixed Restore defaults in Advanced App Launcher.

- Fixed a bug where an active workspace was not highlighted in Workspace preferences menu.

- Fixed a bug where a trailing slash on documentRoot could cause some images to break

- Fixed a bug where a workspace order was wrong after restart.

- enableLegacyLoggerRecipe is now restricted to warnings and errors. This is due to performance issues with higher levels of logging.

- Implemented SearchService to provide Finsemble-style search providers for Workspaces and Launching apps.

- Implemented `Open Logger on Startup` and `Open Logger on Error` settings in Central Logger.

- Fixed a bug in .Net clients where `WindowClient.GetSpawnData()` didn't return data from config.

- Improved performance of `WorkspaceClient.setWorkspaceOrder()`.

- Fixed issue where it wasn't possible to launch Launch Groups from the menu.

- Fixed a bug where a Winform window could break a group on layout reloading.

- Added logging for fsbl-bootstrapper and fsbl-enterBootStage apps for better startup diagnostics

------ 9.8.0-iocd.10

- Fixed bug where `toSpawn` parameter was ignored when spawning a launch group via the API.

- Fixed issue where the download dialog did not appear when downloading a workspace from the user preferences panel.

- Fixed issue where notifications couldn't be deleted.

- Fixed bug where .net apps stuck on shutdown.

- Fixed a bug where window bounds from config were not applied to native apps.

FIN-2245 - Fixed handling of `appd` property in `processAndSet`.

- Fixed a bug where notification default values from config were not applied.

- Fixed a bug where notification subscriptions didn't work properly with filters.

------ 9.8.0-iocd.9

FIN-2145 - Override mouse zoom factors when creating makeInstaller (instead of unionizing)

FIN-2224 - notification filters (fetchHistory) now accept "details" and "title", to match Finsemble behavior. If `_text: true` is set in a rule then a partial search will be conducted.

- Fixed a bug where app config was not applied to the app running from middleware service. (FIN-2082)

- API logging is now at "debug" level instead of "verbose" level. Verbose is reserved for .NET verbose debugging, which causes very large log files.

- Fixed a bug where `disableInnerWindowDragging=true` cmd arg didn't work for WPF apps in IOCD. (FIN-2114)

- Added `persistUrl` support.

- Added `addToWorkspace` support in config.

- `close-requested` event no longer prevents tab groups from closing

FIN-2202 - Added the ability to spawn or show a window and not have it gain focus.

- Channel contexts are now cleared during layout switches.

------ 9.8.0-iocd.8

Fixed bug that prevented code signing from working during makeInstaller process.

FIN-2156 - setting `iocd.getActiveWorkspaceWithoutChanges: true` in Finsemble's manifest will cause getCurrentWorkspace() to _never_ include unsaved changes in the current layout. This improves performance of the call. This can be overridden by the second parameter to getCurrentWorkspace();

FIN-2117 - `customData.isAnApp` now matches legacy Finsemble behavior. It will be true if and only if it comes from apps.json or has been added using appApps() or processAndSet().

FIN-2186 - name "alias" now applies to getActiveDescriptors()

------ 9.8.0-iocd.7

- Fixed bug where apps with lowercase appIds wouldn't convert from legacy Finsemble workspaces

FIN-2186 - If "name" is passed to spawn() then the finsembleWindow object will recognize this name as an alias for iocd's actual window id. This is achieved by storing #finsemble-windowName: name in the window's context. This works for wrap(), finsembleWindow.name, finsembleWindow.windowName. It does not work for finsembleWindow.windowIdentifier or WindowClient or any other legacy window naming fields.

FIN-2176 - customFDC3 recipe converted to synchronous functions to match original implementation

FIN-2101 - Improved app startup time performance by caching app config

- Custom user titles (in tabs) now persist between restarts

FIN-2126 - Directional linking settings now persists in workspaces

Adding `alphabetizeApps` to the manifest's `iocd` section will force apps to be alphabetized in the app launcher.

FIN-2181 - `dependencies` field now has limited capabilities. This will work if it is in the same boot stage as the other dependency and if there are no dependencies on dependencies. This is accomplished simply by incrementing the runPriority (which can always be manually overridden by adding an "io.Connect" manifest)

FIN-2185 - `yarn finsemble launch --iocd` and `yarn finsemble server --iocd` will now work as expected

FIN-2186 - Percentage height and width are now computed correctly for position "relative" when spawning

FIN-2143 - Simplified use of javascript adapter. See example usage above.

FIN-2190 - Added polyfill of Finsemble installer logging config.

FIN-2101 - addApps() now passes through a gate which prevents multiple simultaneous calls from interfering with each other

FIN-2179 - Extend allowedNavigationUrls to accept origin entries as well as host entries.

FIN-2184 - Notification timeout mapped correctly (was mis-mapped seconds to milliseconds)

Added ability to set `fsbl-` prefix for .net apps arguments.

FIN-2167 - Fixed an issue where tab groups could break on restart in certain conditions.

(.4, .5, .6 skipped due to problems with npmjs)

------ 9.8.0-iocd.3

Fixes bug in 9.8.0-iocd.2 that prevented workspace conversion

FIN-2082 - Add the `launchedAsSeparateProcess=true` cmd argument to fix the bug where .net app wasn't saved in workspace if started as separate process.

FIN-2117 - Fixed a bug where .net window title didn't match with title from app config.

------ 9.8.0-iocd.2

Note: Due to a one-time conversion of channel names to match Finsemble, you must install the new IOCD version otherwise your workspace channel settings will be lost when saving workspaces.

FIN-2176 - Channels are now mapped exactly to Finsemble channels, including colors, names and ordering. This is accomplished by providing a custom channels.json with the iocd installer. Finsemble workspaces will map directly to iocd layouts with these channel names. (Note: If the channel names are changed in channels.json then existing workspaces will lose their channel settings, but end users can then reset these and re-save the workspace.)

FIN-2176 - fdc3.getSystemChannels() now returns channels in sorted order.

FIN-2099 - backgroundThrottling is now enabled by default in converted app configs (to match Finsemble default.)

Refactor of pubsub for improved performance

FIN-2175 - Added to .net core client ability to retry connection to IOCD if it is not established in 20 sec.

FIN-2175 - Added `iocd.apiToLogFile` global config parameter to include IOCD .net client logs into Finsemble.dll log file.

FIN-2114 - Fixed a bug where .Net `RouterClient` got stuck while connection to IOCD.

FIN-2171 - Made it so there can be multiple event handlers for each workspace event.

------ 9.8.0-iocd.1

FIN-2097 - CSP directives that could interfere with iframes and other desktop operations are now automatically removed.

FIN-2026 - MultiDirectional channels (linker) is now supported. To enable this mode, in project.json add io.Connect->remoteConfigs->system.json->windows->channelSelector->type: "directionalMulti". (This can also be added on a per app basis by adding an io.Connect manifest to the app config.)

FIN-2139 - Notification toggle now works correctly.

------ 9.7.2-iocd.3

env-allowlist is now supported

processAndSet() was fixed so that it never creates duplicate entries.

------ 9.7.2-iocd.2

Improved .net logging system. Now logs from .net apps are sent to Central Logger in background thread by chunks.

Added log records to track IOCD connection status in .net Client.

FIN-2148 - Fixed setZoomFactor so it doesn't return until the zoom has been applied.

FIN-2157 - Fixed getMonitorInfoAll return. whichMonitor and position were returning incompatible values.

FIN-2082 - Added `disableInnerWindowDragging` cmd argument for .net client to prevent inner window moving inside IOCD frame.

FIN-2114 - Fixed a thread blocking issue for some .net apps when running with IOCD.

FIN-2166 - System level preloads (from "extensions" config entry) are now properly applied.

FIN-2156 - `getActiveWorkspace(null, true)` can now be used to produce a quick response that doesn't hang during a layout switch. The difference is that this will not include any unsaved changes to the workspace.

------ 9.7.2-iocd.1

Note: Version numbers are now synchronized to the official iocd version that they are intended to run against. The final digit may increment between official iocd releases to indicate a patch release of the polyfill.

Fixed bug that prevented project.json overrides for system.json from being applied at launch time. The "io.connect" overrides entry in project.json will now be applied at launch time. If there is a matching launch config with overrides, then those overrides will be applied. If there are no overrides in the launch config, then overrides from the best matching installer config will be used.

Multi-channel linking is now available in .net client.

FIN-2117 - Added `spawnData` and `preload` fields to getActiveWindowDescriptors(). Also fixed `appId` to point to the app name instead of the window id.

FIN-2145 - Fixed bug that prevented custom system.json from being applied during makeInstaller-iocd.

FIN-2143 - Added polyfilled javascript adapter

------ Beta 45

FIN-2139 - "notification-ui" responder is now available for opening/closing notification panel.

FIN-2141 - windows with FSBLHeader: false no longer appear in the taskbar.

FIN-2039 - Removing and renaming workspaces with spaces now works as expected

FIN-2142 - Implemented isAlwaysOnTop.

------ Beta 44

Fixed a bug where .NET IOCD logs overflowed the Central Logger.

FIN-2117 - Fixed a bug where .NET client spawned component with titlebar when `"FSBLHeader": false`.

FIN-2117 - getActiveDescriptors() results now include minimal customData.appConfig.

FIN-2011 - logToDiskLevels is now implemented. These logs will be in logs/applications/central-logger.

FIN-2017 "name" field in appConfig now displays in tab title area (when there is no document.title)

FIN-2125 iocd workspaces are now saved and restored from storage adapters (as part of a global layout)

------ Beta 43

FIN-2039 - Workspace initialization now matches previous Finsemble behavior.
    - Any workspaces in workspaces.json (finsemble.workspaces) will automatically be added to the user's workspace list, even if they delete these workspaces
    - If workspaces.json is not provided then "Default Workspace" will automatically be added
    - If workspaces.json contains an empty array, then "Empty Workspace" will be added _only if the user doesn't have any other workspaces_

------ Beta 42

FIN-2117 - Fix regression in window positioning

FIN-2130 - Ability to run bat scripts

FIN-2126 - Fixed saving of tabbed windows to storage adapter

FIN-2101 - Fixed a bug where legacy Finsemble dll connects to IOCD gateway.

------ Beta 41

Fixed bug in Beta 40 that prevented native apps from being launched.

FIN-2039 - showDialog() now utilizes titlebar

FIN-2117 - Promise.allSettled() is no longer relied upon by FSBL API

------ Beta 40

FIN-2082 - Fixed height shrink in `FitIocdWindowSize` function.

FIN-2039 - WorkspaceClient.import() will now import duplicate workspaces by incrementing the suffix

FIN-2039 - showDialog() now positions correctly

Multi-channel linking is now enabled (directional linking still being developed to work with multi)

FIN-2095 - Implemented heartbeats

FIN-2103 - Workspace metadata (ids and position) were accidentally being overwritten switch switching workspaces

FIN-2082 - Fixed height shrink in `FitIocdWindowSize` function.

FIN-2114 - Fixed a bug where getting config for .net window blocks the app initialization.

------ Beta 39

Please upgrade iocd to: [9.7.0.1](https://interop.io/download/?path=/install/enterprise/finsemble-9.7/stable-offline/9.7.0.1/ioConnectDesktopInstaller&type=b)

FIN-2017 - Electron mirror format is now supported for downloading iocd. Set IOCDINSTALLER in project.json (or iocd_mirror environment variable) to https://username:password@url/path. This will download the installer from that location when using makeInstaller-iocd.

FIN-2046 - fixed the ProcessExit event for native windowless applications.

FIN-2081 - Restart now works. io-connect-desktop-inner.exe is now renamed with the executable name.

FIN-2039 - Fixed the value for error returned by `WorkspaceClient.createWorkspace`.

Fixed issue with Export button on Workspaces Preferences panel.

------ Beta 38

Please upgrade iocd to: [9.7.0.0](https://install.interop.io/install/enterprise/finsemble-9.7/stable/9.7.0.0/ioConnectDesktopInstaller.exe)

FIN-2017 - Zip bundle now available in iocd directory. Use `yarn finsemble internal merge` to assemble the zip file. Then you can point project.json `iocdInstaller` to this zip for use in makeInstaller-iocd.

FIN-2084 - Added support for finsemble.startuptimes router responder

FIN-2081 - addToWorkspace app config is now the default with calling spawn()

FIN-2036 - fdc3.open and fdc3.raiseIntent now support "name" for backward compatibility in .NET apps

------ Beta 37

FIN-2017 - fsbl-service and central logger will now load automatically from a location that is _relative_ to the manifest url. This is accomplished via a bootstrapper app which loads very early in the boot sequence and which is now responsible for launching fsbl-service and central logger. The "build" folder **must** be two levels up (../..) from the location of the manifest file.

FIN-2039 - createWorkspace (New Workspace from menu) now creates an empty workspace in storage adapter

FIN-2034 - getZoomFactor now returns object instead of number

FIN-2081 - Added/fixed windowOptions component.type, custom, isAnApp

FIN-2007 - Fixed command line parsing of manifest
