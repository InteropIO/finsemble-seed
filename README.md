...
Please let us know if you encounter any problems. The first line of troubleshooting is generally to provide a copy of `application.log`. This can be found in AppData/io.Connect/io.Connect Desktop/User Data/ENV-REGION/logs.

More difficult problems may require a console capture from the `fsbl-service` app. To accomplish this, right click iocd the system tray icon and choose "Applications". Find "fsbl-service" and open the devconsole (the "script" icon). This will show the Chromium dev tools for this service. From the "Default levels" pulldown, add "verbose". You can then look for JS errors or copy & paste the console output.

If you're having a problem with a specific app, please provide us with the application config.
