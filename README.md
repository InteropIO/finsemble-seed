...
This version of the Finsemble Seed project is compatible with IO.Connect Desktop (iocd) version [**9.8.0.2**](https://install.interop.io/install/enterprise/finsemble-9.8/stable-offline/9.8.0.2/ioConnectDesktopInstaller.exe) (or [download zip bundle](https://install.interop.io/install/enterprise/citi-9.8/stable/9.8.0.2/ioConnectDesktopBundle.zip)). It also continues to support the legacy Finsemble container (FEA). With this seed version you can run your Finsemble project against either container.
...
A polyfilled JS adapter can be found at @interopio/fsbl-integration/packages/api-polyfill/dist/finsemble-javascript-adapter.js. Include this into your browser apps via a script tag (you will need to host it yourself.)

To enable the adapter, you must first add `auth : { username: , password: }` to your system.json. Set username and password to any values. You can do this by adding an "io.Connect" entry to your project.json. See Overriding system.json section above.

In your browser app, then pass these same values to startApp().

Example
```javascript
    await FSBLJSAdapter.startApp({
        // The appId just can't be a launchable web app
        appId: "finsemble-javascript-adapter",
        // These must be set up in system.json auth
        username: "hoohoo",
        password: "banana"
    });
```
